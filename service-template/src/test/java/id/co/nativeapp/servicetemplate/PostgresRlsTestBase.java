 ...
 * {@code widget} table; the baseline migration's {@code FORCE ROW LEVEL SECURITY} ensures the
 * policy binds even the owning role, so no connection bypasses tenant isolation.
 *
 * <p><strong>Singleton container pattern.</strong> The container is started once in a static
 * initializer and deliberately never stopped per-class (no
 * {@code @Container}/{@code @Testcontainers} JUnit lifecycle). Spring caches a
 * {@code @SpringBootTest} context keyed by its configuration; since both test classes share this
 * identical configuration, the cached context (and its Hikari pool) is reused across them — which
 * is only safe if the underlying container outlives every class. JUnit-managed per-class containers
 * would stop after the first class and leave the reused context pointing at a dead container; the
 * JVM shutdown hook Testcontainers installs (Ryuk) reaps this one at JVM exit.
 */
abstract class PostgresRlsTestBase {
  ...
  /**
   * Start every test from an empty {@code widget} table so the two {@code @SpringBootTest} classes
   * (which share one cached context and one container) are order-independent. Truncating over the
   * admin (BYPASSRLS) connection clears rows of every tenant regardless of any session GUC. The
   * table may not yet exist on the very first invocation before Flyway has run, so the truncate is
   * best-effort.
   */
  @BeforeEach

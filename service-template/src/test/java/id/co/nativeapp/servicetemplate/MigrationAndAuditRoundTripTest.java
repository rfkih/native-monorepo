...
/**
 * Acceptance (b): the Flyway baseline applies against a real PostgreSQL 16 and the
 * sample {@link Widget} entity round-trips with its audit columns populated.
 *
 * <p>The full context boots (so {@link RlsAutoApplyAspect} is active, Hibernate
 * {@code ddl-auto=validate} verifies the mapping against the migrated schema, and
 * Flyway has run). All work happens inside a {@link TenantContext} scope; the
 * persisted row's {@code created_by} comes from that scope's actor (via
 * {@code libs/tenant}'s {@code TenantAuditorAware}) and {@code company_id} from
 * its tenant — proving the audit + tenancy stamping is wired end to end.
 */
@SpringBootTest
class MigrationAndAuditRoundTripTest extends PostgresRlsTestBase {
    ...
    void migrationAppliesAndEntityRoundTripsWithAuditColumns() throws Exception {
        Long id = TenantContext.callAs(TENANT_A, ACTOR_A, () ->
                widgetService.create("first-widget").getId());

 ...
 * (`/api/v1/users/**`, excluding the narrower `/users/me/**` carve-outs this file never calls), so
 * it always uses the PERSONAL bearer (the elevation token on a device terminal; identical to the
 * single login token for a normal `user` login). Shadows the shared `apiFetch` import so every call
 * site below is correct with ZERO per-call changes.
 */

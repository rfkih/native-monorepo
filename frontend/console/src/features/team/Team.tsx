...
type DialogState =
  ...
  | { kind: 'editPages'; member: TeamMember }
    ...
    >
      ...
      {!isSelf ? (
        <div className="flex shrink-0 items-center justify-end gap-1 opacity-0 transition-opacity focus-within:opacity-100 group-hover:opacity-100 max-sm:flex-wrap max-sm:justify-start max-sm:opacity-100">
          ...
          <button
            type="button"
            aria-label={t('team.editPages')}
            title={t('team.editPages')}
            className="rounded-lg px-2 py-1 text-xs font-semibold text-ink-3 hover:bg-ink-50 hover:text-ink focus-visible:outline-2 focus-visible:outline-emerald"
            onClick={() => onEditPages(member)}
          >
            {t('team.editPages')}
          </button>
          <button
          ...
          {visible.length === 0 ? (
            ...
            visible.map((member) => (
              <MemberRow
                ...
                onEditPages={(m) => setDialog({ kind: 'editPages', member: m })}
              />
            ))
          )}
        </Card>

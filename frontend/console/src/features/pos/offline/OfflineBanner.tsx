/**
 * OfflineBanner — a slim, app-global banner (mounted once in app/App.tsx so it is visible on every
 * screen, including the full-screen POS surfaces which render outside the dashboard Shell). Two
 * states only:
...
export function OfflineBanner() {
  ...
  if (offline) {
    return (
      <div
        role="status"
        className="fixed inset-x-0 top-0 z-[70] flex items-center justify-center gap-2 border-b border-amber/30 bg-amber-tint px-4 py-2 text-center text-[13px] font-semibold text-amber-2 print:hidden"
      >
  ...
  return (
    <div
      role="status"
      className="fixed inset-x-0 top-0 z-[70] flex items-center justify-center gap-2 border-b border-emerald-line bg-emerald-tint px-4 py-2 text-center text-[13px] font-semibold text-emerald-2 print:hidden"
    >

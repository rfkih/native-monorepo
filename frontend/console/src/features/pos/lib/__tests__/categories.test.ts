...
describe('deriveCategories', () => {
  it('maps backend categories with a LOWERCASED legacyKey', () => {
    const result = deriveCategories([], [cat({ id: 'c1', name: 'Coffee & Tea' })])
    expect(result).toEqual([{ id: 'c1', name: 'Coffee & Tea', legacyKey: 'coffee & tea' }])
  })
  ...
  it('keeps a free-text category whose categoryId is stale (unknown backend id)', () => {
    ...
    expect(result).toEqual([{ id: 'Snacks', name: 'Snacks', legacyKey: 'snacks' }])
  })
})

...
import { useCreateInvoice, useCustomers, type CreateInvoiceLineBody } from './api'
...
function parseLine(
  line: DraftLine,
  exponent: number,
): { body: CreateInvoiceLineBody; lineTotalMinor: number } | null {
...
/**
 * New invoice — pick a customer, toggle tax, edit line items (add/remove rows), and see a
...
export function NewInvoice() {
  ...
  const mutation = useCreateInvoice({ companyId: company?.companyId ?? '', actor: company?.actor ?? '' })
  ...
  const validLineBodies = parsedLines
    .map((p) => p.parsed?.body)
    .filter((b): b is CreateInvoiceLineBody => !!b)
  ...
  const canSubmit = !!customerId && validLineBodies.length > 0 && !mutation.isPending
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!canSubmit) return
    mutation.mutate(
      { customerId, currency, taxable, lines: validLineBodies },
      {
        onSuccess: (created) => {
  ...
  return (
    <div className="mx-auto flex max-w-[820px] flex-col gap-[18px]">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        <Card className="p-6">
          <Field label={t('ar.newInvoice.customerLabel')} htmlFor="inv-customer">
            {customers.length === 0 && !customersQuery.isLoading ? (
              ...
              >
                <option value="" disabled>
                  {t('ar.newInvoice.selectCustomer')}
                </option>
                {customers.map((c) => (
          ...
          <label className="mt-4 flex cursor-pointer items-start gap-3 rounded-2xl border border-line bg-surface p-4">
            ...
            <span>
              <span className="block text-sm font-medium text-ink">
                {t('ar.newInvoice.taxableLabel')}
              </span>
              <span className="mt-0.5 block text-xs text-ink-3">
                {t('ar.newInvoice.taxableHint')}
              </span>
            </span>
          </label>
        </Card>
          ...
          <div className="flex flex-col gap-3">
            {lines.map((line, idx) => {
              ...
              return (
                ...
                >
                  <Field label={t('ar.newInvoice.descriptionLabel')} htmlFor={`line-desc-${line.key}`}>
                    <TextInput
                      id={`line-desc-${line.key}`}
                      value={line.description}
                      onChange={(e) => updateLine(line.key, { description: e.target.value })}
                      placeholder={t('ar.newInvoice.descriptionPlaceholder')}
                    />
                  </Field>
                  <Field label={t('ar.newInvoice.quantityLabel')} htmlFor={`line-qty-${line.key}`}>
                    <TextInput
                  ...
                  <Field label={t('ar.newInvoice.unitPriceLabel')} htmlFor={`line-price-${line.key}`}>
                    <TextInput
                  ...
                  <Field label={t('ar.newInvoice.lineTotal')}>
                  ...
                  <button
                    type="button"
                    aria-label={t('ar.newInvoice.removeLine', { n: idx + 1 })}
        ...
        <Card className="p-6">
          ...
          <div className="flex flex-col gap-2 text-sm">
            <div className="flex items-center justify-between">
              <span className="text-ink-2">{t('ar.newInvoice.subtotal')}</span>
              <span className="tnum font-mono text-ink">
                {formatMoney(subtotalMinor, currency, locale)}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-ink-2">{t('ar.newInvoice.tax')}</span>
              <span className="tnum font-mono text-ink">
            ...
            <div className="flex items-center justify-between border-t-[1.5px] border-line-strong pt-2">
              <span className="font-semibold text-ink">{t('ar.newInvoice.total')}</span>
              <span className="tnum font-mono text-lg font-semibold text-ink">
                {formatMoney(totalMinor, currency, locale)}
              </span>
            </div>
          </div>
          <p className="mt-3 text-xs text-ink-3">{t('ar.newInvoice.previewNote')}</p>
        </Card>

        {mutation.isError ? (
          <p className="text-sm text-loss">{t('ar.newInvoice.errorTitle')}</p>
        ) : null}

        <div className="flex justify-end gap-3">
          <Button type="submit" disabled={!canSubmit}>
            {mutation.isPending ? t('ar.newInvoice.submitting') : t('ar.newInvoice.submit')}
          </Button>
        </div>
      </form>
    </div>

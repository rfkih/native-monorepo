      ...
      >
        <Card className="reveal max-h-full w-full max-w-sm overflow-y-auto overscroll-contain p-6 text-center">
          ...
          <div className="mt-5 rounded-2xl border-[1.5px] border-dashed border-emerald-line bg-emerald-tint/50 px-4 py-5">
            <p className="mb-2 text-2xs font-bold uppercase tracking-eyebrow text-emerald-2">
              {t('pos.loyalty.giftCard.printableCode')}
            </p>
            <p className="tnum select-all break-all font-mono text-xl font-bold tracking-eyebrow text-ink">
              {result.code}
            </p>
          </div>
...
function KeypadButton({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="tnum flex h-11 items-center justify-center rounded-xl border border-line bg-surface font-mono text-base font-semibold text-ink transition-colors hover:bg-hover active:bg-hover focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald"
    >
      {label}
    </button>
  )

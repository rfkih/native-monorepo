/**
 * mobile-fixtures — the phone harness's API fixtures (IDR, minor units = whole rupiah): a company,
 * a profile, payslips, the till's menu/bills/orders, the opname catalog, statements, ledgers…
 * and `resolveFixture`, the first-match route table that answers every /api/v1/** call from them.
 *
 * Shared by mobile-shots.mjs (the screenshot walk) and overflow-audit.mjs (the text-overflow
 * walk) so both see the SAME data — a finding in one is reproducible in the other.
 */
...
export { COMPANY, PARKED_ORDER, resolveFixture }

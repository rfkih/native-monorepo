...
# built image serves every environment (window.__NATIVE_CONFIG__ → src/lib/config.ts). Runs via the
# nginx base image's /docker-entrypoint.d mechanism.
set -eu
...
EOF
echo "native: wrote runtime config to $TARGET"

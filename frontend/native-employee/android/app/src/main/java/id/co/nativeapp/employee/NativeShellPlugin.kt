 ...
 * itself. Identical to the Till app's NativeShellPlugin (each shell keeps its own copy, per
 * package); web access goes through the console's lib/nativeShell.ts, which treats every method
 * as optional (old APKs simply lack it).
 *
 *  • minimize() — background the app (moveTaskToBack), the "Exit" of the back-guard dialog.
 *  • saveFile() — write a base64 payload into Downloads via MediaStore (<a download>/blob clicks
 *    are silently dead in a WebView). MediaStore.Downloads needs API 29+; older devices get a
 *    typed rejection the web layer explains.
 *  • printPage() — hand the CURRENT page to the Android print framework; window.print() is a
 *    no-op in a WebView, which left "Cetak slip" (the app's only print action) dead.
 */

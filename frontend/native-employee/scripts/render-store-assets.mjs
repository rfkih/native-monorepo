// Render the Google Play graphic assets for the Native Karyawan (Employee) app from the in-app brand:
...
// playwright-core lives in the sibling console workspace (not native-employee's deps) — resolve it there.
...
// Business app's single word to keep the row inside 1024px.
const FEATURE_HTML = `<!doctype html><html><head><meta charset="utf-8"><style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .tile{width:96px;height:96px;border-radius:24px;display:grid;place-items:center;
    background:linear-gradient(135deg,#0e8fab,#064654);box-shadow:0 10px 30px rgba(0,0,0,.28)}
  .name{font-size:70px;font-weight:800;letter-spacing:-.03em;line-height:1}
  .tag{margin-top:28px;font-size:33px;font-weight:600;color:#bfe9f1;letter-spacing:.005em}
  .url{margin-top:15px;font-size:23px;font-weight:600;color:#6bc2d6;letter-spacing:.04em}
</style></head><body>
  <div class="stage">
    ...
    <div class="lockup">
      ...
      <div class="url">emp.native-app.my.id</div>
    </div>
  </div>
</body></html>`

...
| [0073](0073-release-tag-only-image-builds.md) | Images are built on release tags only (nothing consumed the per-merge builds — UAT builds locally); `:latest` tracks the newest release; `master` is the single release line | Accepted |
| [0074](0074-sales-leak-detection.md) | Sales-leak detection — `ingredient_usage_day` becomes the full daily movement ledger `ingredient_stock_day`; a read-only, owner-only report estimates revenue that was never rung, and the estimate never enters the ledger | Accepted |
| [0075](0075-navigation-and-overlay-contract.md) | A navigation and overlay contract for the console — back pops (never pushes a destination), navigation decides scroll, one dialog primitive, chrome never swapped by a Suspense fallback | Accepted |
| [0076](0076-settlement-sources-and-multi-source-payouts.md) | Settle by who pays you, not by how the customer paid — source-dimensioned receivables, one payout with many lines, overdue-per-cadence nudge | Proposed |
| [0077](0077-neutral-ink-brand-replaces-deep-cyan.md) | Neutral ink replaces the deep-cyan brand fleet-wide — primary is maximum contrast against the page and inverts on dark, the three-shade rule retires, green still means profit only | Accepted |
| [0078](0078-the-phone-more-surface-is-a-screen.md) | The phone's "More" surface is a screen, not a modal — a phone login's only navigation gets a route, a history entry and its scroll position back (amends [0075](0075-navigation-and-overlay-contract.md)) | Accepted |
| [0079](0079-the-phone-bill-is-a-deck-not-a-sheet.md) | The phone till's bill is an attached deck, not a sheet you open — one `BillDock` for the walk-in cart and for an open bill, and no "open the cart" step in any flow | Proposed |
| [0080](0080-the-phone-reports-chart-is-the-period-picker.md) | On the phone the three statements are one screen (Laporan) and the twelve-month chart is the period picker — tap a column, the whole report moves; rules and CSV builders shared with desktop, never re-derived | Proposed |
| [0081](0081-the-inventory-reads-in-days-not-quantities.md) | The inventory catalog reads in days, not quantities — stock ÷ a seven-day per-open-day usage rate leads every row, the list ranks by what needs action, Terima is the one row action, sub-screens are routes and the opname history reads through the catalog's units | Proposed |
| [0082](0082-the-phone-home-reads-today-not-the-month.md) | The phone home reads today, not the month — restaurant-service gains a per-day sales read (tendered sales − refunds by outlet-local day, with the sale-time COGS fold), the home folds outlets client-side into a seven-day strip, a same-weekday delta, four day figures, tasks and best sellers; the month lives in Laporan; a books-only login keeps the monthly home | Proposed |
| [0083](0083-the-phone-menu-is-a-work-page.md) | The phone menu is a work page, not a catalog — one row opens into the item's whole editor (name, price, availability, recipe, duplicate, delete), HPP is the recipe's derived sum and never typed, lines are picked from the catalog in base units, search reaches into recipes, chips come from the items' own categories; the desktop tree is untouched | Proposed |
| [0084](0084-a-bill-the-books-can-defend.md) | A bill the books can defend — finance carries the vendor's invoice number (unique per vendor among live bills, 409), invoice date + terms at draft time, a header discount spread by largest remainder, the PPN rate (0/11/12 %), a note, vendor terms and private photo/PDF attachments; the phone form is a checklist that refuses what it cannot defend | Proposed |
| [0085](0085-the-design-tokens-gain-a-scale.md) | The design tokens gain a scale — eight whole-pixel type steps in Tailwind's own names (re-valuing `sm`/`lg`/`xl`/`2xl` onto the design), two trackings, four radii, one scrim, one non-inverting ink; every arbitrary value migrates and `check-design-tokens.sh` keeps them out (finishes 0077's last surfaces + boot skeleton) | Proposed |
| [0086](0086-register-close-refuses-open-bills.md) | The register refuses to close over an open bill — `close()` counts the outlet's OPEN bills under the exclusive `CashWindowLock` and answers 409 `register-session-open-bills` before writing (the close key is not consumed; `BillWriter.open` joins the SHARED side; the close never cancels a bill itself; parked/awaiting-payment orders are not "open"); the expected preview carries `openBillCount` so the close sheet lists the bills and withholds Close. And a POS write carries the strongest credential the login holds — the `'elevated'` bearer target (personal when present, else outlet) fixes the elevated owner whose bill cancel rode the device's cashier token; the order switcher gains a per-row cancel | Proposed |

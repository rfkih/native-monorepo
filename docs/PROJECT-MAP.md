...
| daily stock history / average consumption / "berapa kali dikoreksi manual" | restaurant `inventory/` — `IngredientStockDay` + `IngredientStockDayRepository` (the 3 bucket UPSERTs: `addUsage`/`addReceipt`/`addAdjustment`), day attribution in `inventory/domain/StockLedgerDay.java`, V47, ADR 0074 |

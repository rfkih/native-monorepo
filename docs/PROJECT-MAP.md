...
| **employee-service** | HR: employee/contract/assignment (PII-encrypted, console CRUD + list APIs) + **payroll engine** (gross-to-net, flagged-illustrative statutory; runtime setup/compensation/run-read APIs). Gateway-routed (`/api/v1/employees/**`, `/api/v1/payroll-runs/**`, `/api/v1/payroll-setup/**` — DASHBOARD_ROLES; local port 8084) | EmployeeChanged, AssignmentChanged, PayrollPosted, LaborCostAllocated | OrgUnitCreated/Changed, MetricPublished, PeriodSealed | V1–V6 |

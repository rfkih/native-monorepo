...
| **org-service** | company (immutable base_currency + default_language + **`vertical`**: restaurant \| carwash \| barbershop, LOWERCASE — ADR 0070 moved it up from the org unit), the FLAT org list (`company > outlet` only — the division/business-unit and team levels were removed by **ADR 0070**, which supersedes ADR 0012; `org_unit.parent_id` survives always-NULL for wire/read-model compat), legal_employer, user-outlet assignments, consolidation_group + membership, **ADR 0049 P3a per-outlet device (kiosk) credential** — `device_credential` (AES-256-GCM password, own `PiiCipher`) + `POST/GET/DELETE /api/v1/org-units/{outletId}/device-credential` + `.../reset` (DASHBOARD_ROLES, owner/manager only): mints a Keycloak `cashier`/`actor_type=device` login bound to exactly one outlet via the existing `user_outlet_assignment` writer; excluded from the Team list | CompanyCreated, OrgUnitCreated/Changed/**Deleted**, UserOutletAssignmentChanged, GroupDefined, GroupMembershipChanged | — | V1–V15 |
| **restaurant-service** | 1st vertical: `sale` aggregate + full restaurant POS backend (menu w/ per-item stock = the 86 gate — menu images live in the OBJECT STORE since ADR 0048: convert-on-write to `image_key`, public `/api/media/…` URLs, owner backfill `POST /api/v1/menu/images/migrate`; orders/bills/tables, register sessions ADR 0036/0038 (**ADR 0086 — the close refuses 409 `register-session-open-bills` while any bill at the outlet is OPEN, counted under the exclusive `CashWindowLock`; `GET /register-sessions/{id}/expected` carries `openBillCount`; bill summaries carry `paidLineCount`**), menu-item stocktake, **ingredient inventory + ingredient stock opname ADR 0046** — `inventory` feature: `ingredient`/`ingredient_stocktake(_line)` tables, `/api/v1/ingredients/**` + `/api/v1/ingredient-stocktakes/**`; **ADR 0049 P0–P4 — `sale.sold_by_user_id` seller wire, `OperatorRequiredGuard` device-sale enforcement (`SaleWriter#create`/`#recordInCurrentTx`, `PaymentWriter#recordPendingDigitalInCurrentTx`), and `payment.sold_by_user_id` async operator threading (`PaymentCaptureWriter#capture`)**; recipes/BOM ADR 0050; **ADR 0067 Phase B — `goods_receipt`** table, V43: the priced-goods-receipt idempotency anchor for the `StockReceived` outbox event, closing ADR 0056 accepted-limitation #1; **ADR 0074 — the daily movement ledger** (V47 widens V42`s `ingredient_usage_day`; NOT renamed — a rename is non-backward-compatible DDL and would break rolling rollback, ADR 0057): the per-(ingredient, day) MOVEMENT LEDGER — `qty_used` / `received_qty` / signed `adjustment_qty` / reserved `waste_qty` / `closing_qty` plus `receipt_count` + `adjustment_count`, UPSERTed by every stock-moving writer in the same transaction as the movement; reads at `GET /api/v1/ingredients/stock-history` + `/{id}/stock-history`; **ADR 0074 — `integrity` feature**: the READ-ONLY sales-leak report at `GET /api/v1/sales-integrity/report` (OWNER-only at the gateway, the only owner-gated restaurant route) — no table, no event, no GL impact; 14 detectors over existing rows (9 outlet-level + 5 per-person), headline is a low/high RANGE) | SaleRecorded, StocktakeCompleted (menu-item AND ingredient flows), RegisterSessionClosed, StockReceived, … | EntitlementGranted/Revoked, UserOutletAssignmentChanged, InventoryPurchaseRecorded (ADR 0072 — priced goods receipt per line, keyed on line_id), … | V1–V47 |
...
| **finance-service** | the consolidation core: dimensional ledger, P&L, FX, group consolidation. **The big one.** **ADR 0067 Phase B (V54)** — `bill_line.is_inventory` per-line routing flag (the seam `BillWriter`'s EXPENSE_NET/INVENTORY_NET split reads). **ADR 0072 (V58)** — `companyexpense` feature: the first-party company-expense input (`/api/v1/company-expenses`, GENERAL by gl_hint / INVENTORY → 5100 HPP periodic or 2050 GRNI perpetual), whose INVENTORY submits emit `InventoryPurchaseRecorded` for restaurant to receive stock; **P4 (V59)** — `bill_line` ingredient linkage: posted bills with ingredient-linked Persediaan lines emit the same event, and the periodic split routes inventoryNet to 5100 HPP | ConsolidationClosed, TrialBalancePublished, JournalEntryPosted, InventoryPurchaseRecorded | SaleRecorded, ExpenseRecorded, PayrollPosted, LaborCostAllocated, GroupDefined, GroupMembershipChanged, TrialBalancePublished | V1–V59 |
| **payment-service** | QRIS payment modes + PSP charge lifecycle (ADR 0045): `payment_settings` (mode MANUAL\|STATIC\|GATEWAY, merchant static QRIS image — object-store-backed since ADR 0048, merchant-own Midtrans creds — AES-encrypted), `payment_charge` (dynamic QRIS per transaction, settled by the signed inbound Midtrans webhook at `/api/v1/psp-webhooks/midtrans/{companyId}`; `payment.charge.settled{source=webhook\|sync}` tells a live webhook from a dead one masked by `/sync` polling) | PaymentChargeSucceeded, PaymentChargeExpired | — | V1–V6 |
...
| ingredient UNITS (why a recipe won't take a number) | a recipe is typed in the ingredient's BASE unit (`menu/lib/recipeQty.ts`); `kg`/`liter` are DISPLAY labels over `g`/`ml` (`inventory/lib/units.ts`); `pack` is a purchase container, NOT a base unit — buy by the pack via `pack_size` (V46). Stranded ingredients move with `POST /api/v1/ingredients/{id}/convert-unit` (`Ingredient#convertUnit` + the recipe/ledger rescales, OPS_ROLES) |
| ingredient inventory / stock opname (bahan) | restaurant `inventory/` (catalog + receive/set + ingredient stocktake → reuses `StocktakeCompleted`), console `frontend/console/src/features/inventory/` — routes `/inventory`, `/inventory/:id`, `/new`, `/:id/edit`, `/:id/convert`, `/history[/:stocktakeId]` (`IngredientManagement.tsx` host, `InventoryCatalog.tsx` days-left list, `lib/catalogView.ts` rate/class/ranking rules, `QtyKeypadSheet.tsx` receive/set, `StocktakeHistory.tsx` bodies shared with the opname's `StocktakeHistorySheet`, ADR 0081) + `features/stocktake/` (`StocktakeSheet.tsx` count sheet + `lib/stocktakeMarks.ts` checked-status, `inventory/lib/countKeypad.ts` + `inventory/QtyKeypad.tsx` shared pad, `lib/stocktakeVarianceGuard.ts` ADR 0068), ADR 0046 |
| the phone menu work page (manajemen menu di ponsel) | console `frontend/console/src/features/menu/` — `MenuPhone.tsx` (below 640px, switched in `MenuManagement.tsx`: search over item + ingredient names, category chips from the items' `category` strings, one open row = the item's editor: name/price PATCH on commit, availability `/86`·`/un-86`, recipe lines picked from `/api/v1/ingredients` + base-unit keypad → full-replace `PUT /menu/{id}/recipe`, client-side duplicate, soft delete), `lib/menuView.ts` (pure: chips, filter, summary, stock meta, margin, line cost, PUT bodies, price parser), ADR 0083 |
| the phone vendor-bill form (Tagihan baru) / AP bill attachments | finance `ap/` — V69 `bill.vendor_invoice_number` (partial unique per vendor among live bills → 409 `bill-duplicate-invoice`), `bill_date` + `term_days` at draft, `discount_minor` (spread by `DiscountAllocation`, largest remainder), `tax_bp` 0/1100/1200, `note`, `vendor.payment_term_days`; `bill_attachment` + `GET/POST/DELETE /api/v1/ap/bills/{id}/attachments[/{aid}]` (`BillAttachmentWriter/Reader`, `libs/media-storage`, MinIO user `finance`); console `features/ap/NewBillPhone.tsx` (below 640px, switched in `NewBill.tsx`), `lib/newBillForm.ts` (pure: parse lines, totals, reconciliation, due date, duplicate, checklist), `BillAttachments.tsx`, `inventory/IngredientPickerSheet.tsx` (shared picker), ADR 0084 |
| the phone home / "omzet hari ini" (today's sales per day) | restaurant `sale/` — `GET /api/v1/sales/daily?businessId&from&to` (`SaleRepository#findDailySummary`: one row per Asia/Jakarta day in an inclusive window — net = tendered sales − refunds by `refunded_at`, transaction count, Σ `cogs_minor` + costed count, illustrative flag; POS_ROLES via `/api/v1/sales/**`, outlet-guarded, ≤ 92 days), console `frontend/console/src/features/dashboard/` — `DashboardPhone.tsx` (today home, POS-capable logins), `DashboardPhoneMonthly.tsx` (the monthly fallback for a books-only login), `todayApi.ts` (per-outlet `useQueries` fan-outs keyed like the POS hooks), `lib/todayView.ts` (pure: week strip, same-weekday delta, avg bill, gross margin, top-item merge, tasks), ADR 0082 |
| sales-leak / fraud detection ("penjualan tidak tercatat") | restaurant `integrity/` — `SalesIntegrityRepository` (every detector's native query), `LeakEstimator` (the pure, float-free shortfall→revenue allocation), `OperatorOutliers` (the equally float-free per-person rate test, compared against the REST of the outlet), `SalesIntegrityReader` (assembles + folds the range), `config/SalesIntegrityAdvice` (422 mixed-currency); gateway `salesIntegrityRoute` (OWNER_ROLES); console `frontend/console/src/features/integrity/` (`SalesIntegrity.tsx` + `salesIntegrityApi.ts`, route `/sales-integrity` behind `isOwner`, `salesIntegrity.*` i18n keyed off the machine signal type); ADR 0074 |
| daily stock history / average consumption / "berapa kali dikoreksi manual" | restaurant `inventory/` — `IngredientStockDay` + `IngredientStockDayRepository` (precise Java names over the historical `ingredient_usage_day` columns) (the 3 bucket UPSERTs: `addUsage`/`addReceipt`/`addAdjustment`), day attribution in `inventory/domain/StockLedgerDay.java` over the shared `config/OutletZone.java` (the ONE outlet-local zone: ledger day + register business date + leak-report dates), V47, ADR 0074 |

...
```json
{
  ...
  "fields": [
    ...
    {"name": "cash_sales_minor", "type": "long", "doc": "Sum of CASH physically taken into the drawer during the session window, minor units: cash-collected sale portions (the customer-pays amount net of any gift-card-redeemed part) PLUS cash gift-card sales (drawer money even though they are a liability, not revenue). Producer-computed from its own rows; carried so the consumer can assert the reconciliation identity."},

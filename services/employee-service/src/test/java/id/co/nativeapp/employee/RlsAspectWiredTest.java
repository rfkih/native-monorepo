 ...
 * employee-service persistence context — an in-memory H2 {@link DataSource} + JPA — and loads the
 ...
 * PII-encrypted) entities. No PostgreSQL, no Spring Boot boot — pure wiring assertion. RLS
 * *behaviour* stays the Testcontainers isolation suites' job.
 */
class RlsAspectWiredTest {
  ...
  void rlsAutoApplyAspectBeanIsWiredFromLibsTenant() {
    new ApplicationContextRunner()
        ...
        .withPropertyValues(
            "spring.datasource.url=jdbc:h2:mem:employee-rls-aspect-wired;DB_CLOSE_DELAY=-1",
        ...
        .run(
            context -> {
              ...
              assertThat(context)
                  .as("the auto-applied RLS aspect (rule 5) must be wired into employee-service")
  ...
  /**
   * Scans ONLY the out-of-app-base-package {@link SampleEntity} fixture — deliberately NOT the
   * {@code id.co.nativeapp.employee} tree — so the persistence unit builds without Hibernate
   * picking up the service's real entities or the {@code @Converter} {@code PiiAttributeConverter}
   * (whose {@code PiiCipher} is absent here), and so the real {@code @SpringBootTest} entity scan
   * never sees this fixture (which would fail {@code ddl-auto: validate} on a missing table).
   */

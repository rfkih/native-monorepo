...
/**
 * Request-edge tenant binding for the employee HR API — a minimal DEV stand-in for the JWT/gateway.
 ...
 * service is exposed.
 *
 * <p><strong>Consumer path is unaffected.</strong> The {@code OrgUnitCreated}/{@code
 * OrgUnitChanged} {@code @KafkaListener} binds its own tenant scope from the event's {@code
 * company_id} (not from an HTTP header); this filter only governs the inbound {@code
 * /api/v1/employees} read/write paths.
 *
 ...
 * company_id} claim, which is a UUID; the outbox {@code company_id} column is {@code UUID}. So this
 * filter validates the tenant id is a UUID at the EDGE: a missing, blank, or non-UUID {@code
 * X-Company-Id} (or a missing/blank actor) is rejected with {@code 400} and NO scope is bound.
 * Probe endpoints are exempt (see below).
 */
@Component
@Profile("dev")

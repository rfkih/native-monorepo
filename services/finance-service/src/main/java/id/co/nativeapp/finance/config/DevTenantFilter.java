...
/**
 * Request-edge tenant binding for the revenue query API — a minimal DEV stand-in for the
 * JWT/gateway. It reads the tenant ({@code company_id}) and actor from the {@code X-Company-Id} and
 * {@code X-Actor} request headers and wraps the downstream work in {@link
 * TenantContext#callAs(String, String, java.util.concurrent.Callable)}, exactly where the gateway
 * will later bind the validated JWT's {@code company_id} claim and {@code sub}. It performs NO
 * authentication and trusts the headers — it is explicitly a placeholder and must be replaced by
 * real JWT validation before this service is exposed.
 *
 * <p><strong>Consumer path is unaffected.</strong> The {@code SaleRecorded} {@code @KafkaListener}
 * binds its own tenant scope from the event's {@code company_id} (not from an HTTP header); this
 * filter only governs the inbound {@code GET /api/v1/revenue} read path.
 *
 ...
 * company_id} claim, which is a UUID. So this filter validates the tenant id is a UUID at the EDGE:
 * a missing, blank, or non-UUID {@code X-Company-Id} (or a missing/blank actor) is rejected with
 * {@code 400} and NO scope is bound. Probe endpoints are exempt (see below).
 *
...
public class DevTenantFilter extends OncePerRequestFilter implements Ordered {
  ...
  protected void doFilterInternal(
    ...
    if (!isUuid(companyId)) {
      // The tenant id MUST be a UUID (the production company_id claim is a UUID).

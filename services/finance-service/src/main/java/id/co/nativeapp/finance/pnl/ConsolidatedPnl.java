 ...
 *
 * <p>As each {@link id.co.nativeapp.finance.revenue.LedgerPosting} lands, its amount is added onto
 * the matching leg ({@code revenue_minor} or {@code expense_minor}) in the SAME transaction via the
 * atomic upsert in {@link PnlReadModelWriter}, so the read model is always consistent with the
 * ledger (CQRS: post on write, this is the aggregate-on-read projection, maintained eagerly so the
 * read is a single-row lookup).
 *

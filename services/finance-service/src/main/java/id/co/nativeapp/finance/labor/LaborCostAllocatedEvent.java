 ...
 *
 * <p>Each event becomes exactly one EXPENSE {@link id.co.nativeapp.finance.revenue.LedgerPosting}.
 * The {@code glAccount} is treated as a HINT (finance re-resolves the canonical account, CQRS);
 * {@code amount} is the bucket's aggregated employer-borne labor cost as {@link Money} (never a
 * float, rule 8). {@code period} is the run's AUTHORITATIVE period (not derived from {@code
 * occurredAt}); {@code occurredAt} drives only the effective mapping_rule version.
 *

...
/**
 * THE auto-applied RLS guarantee (rule 5), cloned from the proven service shape.
 *
 ...
 *
 * <p>On finance's consumer path the tenant is bound by wrapping each handler in {@code
 * TenantContext.callAs(event.companyId, "finance-consumer", ...)} (the inbound tenant comes from
 * the event, not a request), so the {@code @Transactional} posting + read-model update run under
 * the event's tenant and the RLS {@code WITH CHECK} passes on every insert.
 *

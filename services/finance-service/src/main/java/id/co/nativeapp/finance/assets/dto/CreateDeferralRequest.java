...
public record CreateDeferralRequest(
    @NotNull @Pattern(regexp = "PREPAID_EXPENSE|DEFERRED_REVENUE", message = "kind must be PREPAID_EXPENSE or DEFERRED_REVENUE")
        String kind,
    @NotBlank @Size(max = 255) String description,
    @Min(1) long totalMinor,
    @Min(1) @Max(600) int months,
    @NotBlank @Pattern(regexp = "\\d{4}-(0[1-9]|1[0-2])", message = "startPeriod must be a valid YYYY-MM month")
        String startPeriod,

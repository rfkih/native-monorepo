...
public interface LedgerPostingRepository extends JpaRepository<LedgerPosting, UUID> {
   ...
   * account_type}; {@link id.co.nativeapp.finance.withinclose.service.TrialBalanceReader} then
   * FAILS LOUD on it (the close fails rather than understating the books). The mapping-rule FK
   * prevents an unmapped account today, so this is a guard, not an expected path.
   */
  @Query(
      value =
          """

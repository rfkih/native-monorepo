...
/**
 * Orchestrates posting a consumed {@code SaleRecorded} to the ledger — the consumer-side core of
 * M1.5.
 *
 ...
 * the proxied {@link RevenuePostingWriter} so the {@code @Transactional} advice and the auto-RLS
 * aspect engage under that tenant. The actual transactional unit of work — dedupe + post +
 * accumulate — lives in the writer (a separate bean so the Spring proxy is not bypassed by
 * self-invocation).
 */
@Service
public class RevenuePostingService {
  ...
  private final RevenuePostingWriter writer;

  public RevenuePostingService(RevenuePostingWriter writer) {
    this.writer = writer;
  }

  /**
   * Handles one decoded {@code SaleRecorded}, idempotently. Binds the event's tenant for the
   ...
   */
  public boolean handle(SaleRecordedEvent event) {
    try {
      ...
      throw new IllegalStateException("Failed to handle SaleRecorded", e);
    }
  }
}

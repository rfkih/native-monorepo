...
/**
 * One billed line of an {@link Invoice}. {@code line_total = unit_price × quantity} is computed
 * once on the server (never a float — {@link Money#multiply(long)} is exact) and stored, so the
 * invoice total is authoritative and immutable to client tampering.
 *
 * <p>Quantity is a whole unit in Phase 1 (fractional quantities are a later enhancement). Money is
 ...
 *
 * <p>Extends {@link Auditable}; under the {@code invoice_line} RLS policy (V24).
 */
@Entity
...
public class InvoiceLine extends Auditable {
   ...
   *
   * @param invoiceId the owning invoice id
   * @param lineNo the 1-based ordering within the invoice
  ...
  public static InvoiceLine of(
      UUID invoiceId, int lineNo, String description, int quantity, Money unitPrice) {

 ...
 * full entity. Lives in its own {@code projection} package — a read model is neither the
 * write-side {@code domain} entity nor a request/response {@code dto}.
 */
public interface WithinCompanyCloseView {

  /** Deliberately returns {@code String} over a UUID column; PostgreSQL casts uuid→text transparently
   * for the projection, and {@code CloseHistoryResponse.closeId} is a String on the JSON surface. */
  String getId();

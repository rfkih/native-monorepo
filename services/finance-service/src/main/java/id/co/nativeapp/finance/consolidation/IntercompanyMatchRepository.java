  ...
  @Query(
      ...
      SELECT count(m) FROM IntercompanyMatch m
       WHERE m.groupId = :groupId
         ...
         AND m.closeRunSeq = :closeRunSeq
         AND m.state IN (
           id.co.nativeapp.finance.consolidation.IntercompanyMatchState.UNMATCHED,
           id.co.nativeapp.finance.consolidation.IntercompanyMatchState.AMOUNT_MISMATCH,
           id.co.nativeapp.finance.consolidation.IntercompanyMatchState.MALFORMED)
      """)

 ...
 *
 * <p><strong>Tenant = the bound company.</strong> Like the within-company close, the return is filed
 * as the company itself — the tenant the gateway / dev filter already bound from the JWT before the
 * controller ran. So this service does not re-bind a tenant; it invokes the proxied writers, which
 * run under the already-bound company scope (RLS engages on the GL read + the {@code tax_filing}
 * write). Delegating through this thin bean (not calling the writers directly from the controller)
 * keeps the layering uniform with the rest of finance.
 */
@Service
public class TaxFilingService {
  ...
  /**
   * Files the PPN return for {@code (the bound company, period)}; returns the filing id + whether it
   * was freshly filed (so the controller can return 201 vs an idempotent 200).
   */

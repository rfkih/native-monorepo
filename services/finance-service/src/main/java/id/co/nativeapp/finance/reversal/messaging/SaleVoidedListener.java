...
/**
 * Consumes {@code SaleVoided} off Kafka and posts the reversal contra entry to the ledger (ADR
 * 0006, slice 4).
 *
 ...
 * SaleVoidedSchema}. No Confluent / Schema Registry serde is used, consistent with how the outbox
 * stores events.
 *
 ...
 * SaleVoided.DLT} by the container's error handler ({@link
 ...
 */
@Component
public class SaleVoidedListener {
  ...
  private static final Logger log = LoggerFactory.getLogger(SaleVoidedListener.class);

  private final ReversalPostingService reversalPostingService;

  public SaleVoidedListener(ReversalPostingService reversalPostingService) {
    this.reversalPostingService = reversalPostingService;
  }

  /**
   * Handles one record from the {@code SaleVoided} topic. Decodes the raw Avro bytes, derives the
   * event id from the header, and delegates to the idempotent reversal posting service.
   *
   * <p>A record whose value is NOT a valid {@code SaleVoided} Avro payload (garbage bytes, a
   ...
   * SaleVoidedDecodeException} which the container's error handler treats as non-retryable and
   * routes straight to {@code SaleVoided.DLT} — money is preserved for inspection on the DLT, not
   * silently dropped, and no reversal ledger posting is written.
   */
  @KafkaListener(
      topics = SaleVoidedSchema.TOPIC,
      containerFactory = "kafkaListenerContainerFactory")
  public void onSaleVoided(ConsumerRecord<String, byte[]> record) {
    UUID eventId = eventIdOf(record);
    SaleVoidedEvent event = decode(eventId, record);
    boolean posted = reversalPostingService.handleVoid(event);
    if (!posted) {
      log.debug("Skipped re-delivered SaleVoided eventId={} (already processed)", eventId);
    }
  }

  /**
   * Decodes the raw Avro value into a {@link SaleVoidedEvent}, converting ANY decode failure into a
   * non-retryable {@link SaleVoidedDecodeException} so a poison payload deterministically lands on
   * the DLT rather than retrying forever.
   */
  private static SaleVoidedEvent decode(UUID eventId, ConsumerRecord<String, byte[]> record) {
    try {
      return SaleVoidedSchema.decode(eventId, record.value());
    } catch (RuntimeException decodeFailure) {
      throw new SaleVoidedDecodeException(
          "Failed to decode SaleVoided payload at "
              + record.topic()
              + "-"
              + record.partition()
              + "@"
   ...
   * treats as non-retryable and routes straight to {@code SaleVoided.DLT}.
   */

 ...
 * tables; the migrations' {@code FORCE ROW LEVEL SECURITY} ensures the policy binds even the owning
 ...
 * {@code @SpringBootTest} context keyed by its configuration; since the test classes share this
 * identical configuration, the cached context (and its Hikari pool) is reused across them — which
 * is only safe if the underlying container outlives every class. The JVM shutdown hook
 * Testcontainers installs (Ryuk) reaps this one at JVM exit.
 */

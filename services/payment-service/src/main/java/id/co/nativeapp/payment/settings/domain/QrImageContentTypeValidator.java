...
/**
 * Verifies a static-QRIS upload's ACTUAL bytes against its DECLARED {@code Content-Type} by
 * magic-byte signature — the declared multipart header is client-supplied and therefore untrusted
 * (the employee-service {@code ReceiptContentTypeValidator} idiom, ported for ADR 0045): a spoofed
 * header (e.g. an executable renamed {@code qris.png}) must not be accepted just because the client
 * claims {@code image/png}.
 *
 ...
 */
public final class QrImageContentTypeValidator {
  ...
  private QrImageContentTypeValidator() {}
   ...
   * @throws InvalidQrImageException if the actual bytes are unrecognised, or disagree with a
   *     non-null declared header (→ 422)
   */
  public static String validate(String declaredContentType, byte[] data) {
    ...
    if (detected == null || (normalizedDeclared != null && !detected.equals(normalizedDeclared))) {
      throw new InvalidQrImageException(declaredContentType, detected);
    }
    return detected;
  }

...
/**
 * AES-256-GCM authenticated encryption for secret-grade columns (rule 6). Ported VERBATIM from
 * loyalty-service's {@code config.PiiCipher} (itself the employee-service mechanism): one instance
 * holds the 256-bit key sourced from {@link PiiEncryptionProperties}; {@link
 * PiiBytesAttributeConverter} delegates to it so the {@code server_key_encrypted} / {@code
 * client_key_encrypted} columns — the merchant's OWN Midtrans credentials (ADR 0045) — are
 * ciphertext at rest and decrypt back transparently on read.
 *
 ...
 * (which is why the display affordance is the separate plaintext {@code server_key_last4} column,
 * written once at save time — the encrypted column itself can never be searched or compared).
 *
 ...
 *
 * <p><strong>BYTEA storage.</strong> {@link PiiBytesAttributeConverter} base64-DECODES {@link
 * #encryptToString(String)}'s output into raw bytes before the {@code BYTEA} write, and
 * base64-ENCODES the stored bytes back before calling {@link #decryptFromString(String)} on read —
 * the loyalty-service column-type deviation, kept identical here.
 */

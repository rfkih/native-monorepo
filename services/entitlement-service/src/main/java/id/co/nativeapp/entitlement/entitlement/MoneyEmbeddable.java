 ...
 * CHAR(3)} on {@code billing_line}; it carries no floating-point field at all.
 *
 * <p>The domain works entirely in {@link Money}; this embeddable exists only at the persistence
 * boundary. {@link #toMoney()} reconstructs the value type (re-validating the currency via {@link
 * Money#ofMinor(long, String)}), and {@link #of(Money)} projects a {@link Money} back down to its
 * two columns. Hibernate requires the no-arg constructor and mutable fields, hence the protected
 * ctor; application code should prefer {@link #of(Money)}.
 */
@Embeddable

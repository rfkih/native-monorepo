 ...
 * end-to-end through a REAL Kafka broker (mirrors {@code StaffReadModelConsumeTest}).
 *
 ...
 * the real {@code @KafkaListener}s into the local read models — proving the WIRE-level path (raw
 * bytes, the byte[] deserializer, the RLS-scoped projection write) actually works, not just the
 * service-layer logic ({@link LoyaltyRefConsumerAcceptanceTest} covers the deeper idempotency/
 * set-if-newer correctness deterministically, without depending on Kafka delivery ordering).
 */
@SpringBootTest

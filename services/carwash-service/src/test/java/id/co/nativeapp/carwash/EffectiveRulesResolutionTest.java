 ...
 * /api/v1/carwash/pricing/effective-rules} preview endpoint (Phase 5 offline mode, ADR 0028)
 * backing method, against a real {@code tax_charge_rule} table (Testcontainers PostgreSQL). Covers
 * a seeded-rule resolution (both rule families, with provenance), the full no-rule fall-through
 * (zeros/nulls INCLUDING {@code currency}), and RLS tenant isolation — mirrors {@link
 * TaxChargeRuleResolutionTest}'s seeding strategy (direct admin/BYPASSRLS inserts; {@code
 * tax_charge_rule} is deliberately NOT truncated by {@link PostgresRlsTestBase#resetTables()}, so
 * every scenario here uses its OWN dedicated tenant id, distinct from that class's {@code
 * 77777777-...} range).
 *

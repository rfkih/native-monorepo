...
import id.co.nativeapp.carwash.outletref.messaging.UserOutletAssignmentChangedSchemas;
...
/**
 * Consumer-driven contract test for the {@code UserOutletAssignmentChanged} event, mirroring the
 * naming convention carwash-service already uses for its other consumed-event contract tests
 * ({@code StaffConsumerContractTest}, {@code EntitlementConsumerContractTest}) — ported and renamed
 * from restaurant-service's {@code UserOutletAssignmentChangedContractTest}.
 *
 ...
 *   <li>A {@link GenericRecord} round-trips through {@link AvroSerde} — the exact path {@link
 *       id.co.nativeapp.carwash.outletref.messaging.UserOutletAssignmentListener} takes off the
 *       wire.
 *   <li>The consumer schema is BACKWARD-COMPATIBLE with the producer schema (docs/EVENT-CATALOG.md
 *       anchor): a carwash reader can decode bytes the org-service producer wrote.
 ...
 * </ol>
 *
 * <p>No Spring context, no database, no Kafka — just Avro schema + serde logic.
 */
class UserOutletAssignmentConsumerContractTest {
  ...
  private static final String USER_ID = "kc-user-attendant-01";

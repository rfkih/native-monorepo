...
/**
 * End-to-end acceptance test for the Phase-3 promotions engine (ADR 0026) over the real carwash
 * quote/checkout/capture flow, a fresh (no seeded tax/SC rule — the V5 seed is scoped only to
 * tenant {@code 11111111-...}) tenant so the math is exact. Ported from restaurant-service's {@code
 * PromotionCheckoutIntegrationTest}, adapted to the ticket flow (checkout collapses straight to a
 * sale — no separate park/pay step) and both tender paths:
 *
 * <ul>
 ...
 *       sale_id} already set (sale id == ticket id, ADR 0023 decision 2);
 *   <li>DIGITAL checkout writes {@code applied_promotion} rows with {@code sale_id = NULL}; capture
 *       stamps it;
 *   <li>{@code carwash_ticket.coupon_id} is stamped;
...
class PromotionTicketIntegrationTest extends KafkaPostgresRedisTestBase {
  ...
  private static final UUID OUTLET = UUID.fromString("99999999-9999-9999-9999-999999999901");
  ...
  void
    ...
    // carwash_ticket.coupon_id is stamped.
  ...
  void digitalCheckoutWritesNullSaleIdThenCaptureStampsIt() throws Exception {
    ...
    UUID packageId =
        TenantContext.callAs(
                ...
                () ->
                    catalogService.createPackage(
                        new CatalogItemCreateRequest(OUTLET, "Basic Wash", null, 40_000L, "IDR")))
            .id();
  ...
  private UUID couponIdColumnAsAdmin(UUID ticketId) throws Exception {
    try (Connection admin =
            java.sql.DriverManager.getConnection(
                POSTGRES.getJdbcUrl(), POSTGRES.getUsername(), POSTGRES.getPassword());
        PreparedStatement ps =
            admin.prepareStatement("SELECT coupon_id FROM carwash_ticket WHERE id = ?")) {
   ...
   * {@code FORCE ROW LEVEL SECURITY} (V8), so the RLS-scoped {@code jdbcTemplate} (no tenant GUC

 ...
 *
 * <p>Each feature package {@code id.co.nativeapp.carwash.<feature>} holds its classes in a layer
 ...
 *
 * <p>The {@code Projection} layer is a required (non-optional) layer because carwash-service now
 * carries a projection package under {@code wash.projection} for the native read queries on {@code
 * WashRepository} (CODE-STRUCTURE §3.3). The {@code repositoryQueriesAreNative} rule fails the
 * build on any JPQL {@code @Query}.
 */
class LayeredArchitectureTest {
   ...
   *
   * <p>The {@code Projection} layer is a required (non-optional) layer because {@code
   * wash.projection.WashView} exists (CODE-STRUCTURE §3.3).
   */
  @Test
  void featureLayersRespectTheLayeredArchitecture() {
    ArchRule rule =
        layeredArchitecture()
            ...
            .layer("Dto")
            .definedBy("..dto..")
            // Projection holds native-query read models (WashView) produced by the repository
            // layer and consumed (mapped to a dto) by the service layer — reachable from neither
            // the controller nor below. CODE-STRUCTURE §3.3.
            .layer("Projection")
   ...
   * other event-driven integration in this fleet) exists to avoid. Carwash does not yet consume the
   * operator token (deferred to P4), but the guard applies fleet-wide from P2 so no future feature
   * introduces the forbidden dependency ahead of that wiring.
   */
  @Test

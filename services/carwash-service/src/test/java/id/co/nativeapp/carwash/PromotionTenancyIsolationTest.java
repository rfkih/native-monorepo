 ...
 * TicketTenancyIsolationTest}'s tenant-pair idiom and restaurant-service's {@code
 * PromotionTenancyIsolationTest} (ported to the ticket flow).
 *
 * <ul>
...
class PromotionTenancyIsolationTest extends KafkaPostgresRedisTestBase {
  ...
  void tenantBCannotUseOrSeeTenantAsCouponOrRule() throws Exception {
    ...
    // Tenant B: its own package, in its own outlet — attempts to quote using tenant A's code.
    UUID packageB =
        TenantContext.callAs(
                TENANT_B,
                ACTOR_B,
                () ->
                    catalogService.createPackage(
                        new CatalogItemCreateRequest(
                            OUTLET, "Tenant B Wash", null, 10_000L, "IDR")))
            .id();
        ...
        TenantContext.callAs(
            ...
            () ->
                ticketService.quote(
                    new QuoteRequest(
                        OUTLET,
                        List.of(new TicketLineInput(ItemType.PACKAGE, packageB, 1)),
                        null,
                        "TENANT-A-CODE")));
        ...
        TenantContext.callAs(
            ...
            () -> {
              CatalogItemResponse packageA =
                  catalogService.createPackage(
                      new CatalogItemCreateRequest(OUTLET, "Tenant A Wash", null, 10_000L, "IDR"));

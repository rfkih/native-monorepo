...
/**
 * Integration tests for {@link OutletAccessGuard} — ported from restaurant-service's {@code
 * order.OutletEnforcementTest} (Phase 5 policy), but exercising the guard DIRECTLY rather than
 * through a checkout/bill write path: carwash-service has no ticket/checkout writer yet (that is a
 * separate task — this port lands only the guard + its read model as the foundation), so this test
 * calls {@link OutletAccessGuard#enforce} from inside a small test-only {@code @Transactional}
 * wrapper bean, mirroring exactly how a future ticket-checkout writer will call it (the guard's
 * javadoc requires the caller's transaction to already have the RLS GUC set).
 *
 ...
 * </ol>
 *
 * <p>The {@link id.co.nativeapp.carwash.config.ActorRolesProvider} reads the {@code X-Roles} header
 * from the current HTTP request. These tests bind a {@link MockHttpServletRequest} directly to
 * Spring's {@link RequestContextHolder} before each test to simulate the roles header the gateway
 * stamps — the same minimal setup restaurant-service's guard test uses: no MockMvc, no HTTP server,
 * no {@code DevTenantFilter}.
 */
@SpringBootTest
class OutletAccessGuardTest extends PostgresRlsTestBase {
   ...
   * the Spring proxy engages the auto-RLS aspect — exactly how a real caller (a future {@code
   * *Writer}) is required to invoke the guard. Mirrors the {@code @TestConfiguration}/{@code @Bean}
   * pattern already used by {@code RecordWashAtomicityTest} in this module.
   */
  @TestConfiguration

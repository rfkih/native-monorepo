package id.co.nativeapp.carwash.promotion.repository;

import id.co.nativeapp.carwash.promotion.domain.Coupon;
import id.co.nativeapp.carwash.promotion.projection.CouponRuleView;
import id.co.nativeapp.carwash.promotion.projection.CouponView;
 ...
 * the bound tenant automatically — no manual {@code WHERE company_id} is required. Ported verbatim
...
public interface CouponRepository extends JpaRepository<Coupon, UUID> {
   ...
   * id.co.nativeapp.carwash.promotion.service.PromotionEngineService PromotionEngineService} needs
   * to validate and apply a coupon's deduction.
   */
  @Query(
      value =
          """
  ...
  /**
   * THE single-use / max-redemption enforcement idiom (V8 migration comment) — one atomic UPDATE
   ...
   * maps that to {@link id.co.nativeapp.carwash.promotion.domain.CouponExhaustedException} (409).
   *

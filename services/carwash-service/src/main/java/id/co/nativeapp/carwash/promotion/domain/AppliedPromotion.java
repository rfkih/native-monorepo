 ...
 *
 * <p><strong>Port note (difference from restaurant).</strong> Restaurant's {@code AppliedPromotion}
 * names this column {@code order_id} (that service has two revenue aggregates — order and bill —
 * sharing the column). Carwash has exactly one: {@code carwash_ticket}. The V8 migration therefore
 * names the column {@code ticket_id} directly (no generic reuse needed), and this field is named
 * {@code ticketId} to match. There is no {@code REFERENCES} clause (this is an independent audit
 * trail, not a CASCADE-owned child) — same as restaurant.
 *

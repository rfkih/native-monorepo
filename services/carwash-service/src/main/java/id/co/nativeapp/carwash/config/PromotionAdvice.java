 ...
 * </ul>
 *
 * <p><strong>Port note (deliberate omission vs. restaurant's {@code PromotionAdvice}).</strong>
 * Restaurant's source class ALSO handles {@code DataIntegrityViolationException} (a duplicate
 * coupon {@code (company_id, code)}) — restaurant has no {@code CatalogAdvice} of its own, so that
 * was a clean addition there. Carwash's {@link CatalogAdvice} ALREADY registers a {@code
 * DataIntegrityViolationException} handler service-wide (409, generic "catalog-item-conflict"

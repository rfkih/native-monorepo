 ...
 * menu_category}-equivalent catalog dimension for {@code wash_package}/{@code wash_addon}, so {@link
 * id.co.nativeapp.carwash.promotion.dto.EvalLine#categoryId() EvalLine.categoryId()} is always {@code
 * null} for a carwash ticket line, which means a {@code CATEGORY}-scoped rule can never match here —
 * the admin API accepts creating one (schema allows it), it would simply never apply. See {@link
 * id.co.nativeapp.carwash.promotion.service.PromotionEngineService PromotionEngineService}.
 */

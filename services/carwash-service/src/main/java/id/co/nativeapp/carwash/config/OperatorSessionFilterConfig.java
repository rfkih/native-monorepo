...
/**
 * Registers the shared {@code libs/security} {@link OperatorSessionFilter} over carwash-service's
 * POS write routes (ADR 0049 P4) — the vertical opt-in the filter's own javadoc calls out (it is
 * NOT auto-registered fleet-wide). Mirrors restaurant-service's identical {@code
 * OperatorSessionFilterConfig} (P2); carwash/barbershop opt in only now, at P4.
 *
 * <p><strong>Scope.</strong> Path-scoped to the ticket routes that quote/checkout/capture a ticket:
 * {@code /api/v1/carwash/tickets/**} — every controller {@code TicketWriter#create}/{@code
...
public class OperatorSessionFilterConfig {
  ...
  private static final String[] URL_PATTERNS = {
    "/api/v1/carwash/tickets", "/api/v1/carwash/tickets/*"
  };

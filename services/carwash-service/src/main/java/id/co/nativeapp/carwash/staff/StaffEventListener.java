 ...
 * local staff read model ({@link Staff}) so the vertical knows its staff (rule 2 — a cached read
 * model, never a sync call).
 *
 ...
 * StaffProjectionWriter}. A record arriving WITHOUT a valid {@code id} header fails closed (see
 * {@link #eventIdOf}) so it is DLT'd.
 *

...
/**
 * Orchestrates applying a consumed {@code PaymentChargeSucceeded} (ADR 0045) to the carwash ticket
 * it settles.
 *
 ...
 * {@link TenantContext#callAs} with a fixed {@code "carwash-payment-charge-consumer"} actor (which
 * lands in the Auditable {@code created_by}/{@code updated_by} of any row the capture writes), then
 * delegates to the proxied {@link PaymentChargeSucceededWriter} so the {@code @Transactional}
 * advice and the auto-RLS aspect engage under that tenant. Mirrors {@code
 * entitlement.service.EntitlementProjectionService} EXACTLY (carwash's established consumer idiom).
 * The transactional unit of work — dedupe + the vertical/reference/state/amount checks + capture —
 * lives in the writer (a separate bean so the Spring proxy is not bypassed by self-invocation).
 */
@Service
public class PaymentChargeSucceededService {

  /** The audit actor for system-driven consumer writes (stamped into {@code created_by}). */
  public static final String CONSUMER_ACTOR = "carwash-payment-charge-consumer";

 ...
 *
 * <p>Emitted by {@link id.co.nativeapp.carwash.ticket.service.OperatorRequiredGuard} when a device
 * (outlet-terminal) actor attempts to ring a ticket with no verified operator session. Detail does
 * not echo any request-derived data.
 */
...
public class OperatorAdvice {
  ...
  public ProblemDetail handleOperatorRequired(
    ...
    problem.setDetail(
        "This outlet terminal requires a verified operator (attendant PIN) session to ring a"
            + " ticket.");

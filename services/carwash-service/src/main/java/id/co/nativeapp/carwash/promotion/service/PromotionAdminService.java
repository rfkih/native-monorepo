...
import id.co.nativeapp.carwash.promotion.dto.CouponPatchRequest;
import id.co.nativeapp.carwash.promotion.dto.CouponResponse;
import id.co.nativeapp.carwash.promotion.dto.PromoRuleCreateRequest;
import id.co.nativeapp.carwash.promotion.dto.PromoRulePatchRequest;
 ...
 * verbatim from restaurant-service (ADR 0026).
 *
 ...
 * carwash's {@code requireStaffWriteRole} (empty-roles-pass semantics: a headerless call is the
 * gateway-less dev recipe / a direct service-layer test, not a real cashier/attendant token).
 *
 * <p><strong>Vertical-prefixed routing (difference from restaurant).</strong> Restaurant's admin
 * endpoints ride the DASHBOARD gateway route (owner/manager enforced at the gateway too). Carwash's
 * {@code /api/v1/carwash/promotions[/coupons]} endpoints ride the existing POS-roles gateway route
 * instead (see {@code PromotionAdminController} class javadoc) — this service-side write guard is
 * therefore the ONLY guard for a write; reads stay open to any authenticated POS caller (a cashier
 * sees the rule/coupon catalog, acceptable — they already see prices).
 *

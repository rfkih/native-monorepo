 ...
 * id.co.nativeapp.carwash.promotion.service.PromotionEngineService PromotionEngineService} resolves
 * at checkout/quote time. Maps to the {@code promo_rule} table (V8__promotions.sql). Ported
 * verbatim from restaurant-service (identical DDL, identical entity shape).
 *
 ...
 * a percent-type rule legitimately carries no amount and vice versa, so the "amount, always
 * present" invariant a {@code MoneyEmbeddable} implies does not hold here (rule 8 — never a float;
 * still true, just not wrapped in {@code Money}).
 *

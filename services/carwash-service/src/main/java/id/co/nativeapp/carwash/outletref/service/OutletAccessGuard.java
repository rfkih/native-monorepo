...
import id.co.nativeapp.carwash.outletref.repository.UserOutletAssignmentRefRepository;
 ...
 * ACTIVELY assigned to (ported verbatim from restaurant-service's {@code outletref} feature — the
 * Phase 5 policy). Intended to be called from every future carwash sale-recording write path (the
 * ticket-checkout writer, not yet built — this port lands only the guard + its read model).
 *

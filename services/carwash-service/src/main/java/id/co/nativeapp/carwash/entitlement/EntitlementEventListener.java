 ...
 * the local entitlement projection ({@link EntitlementProjection}) — the projection the record-wash
 * gate is checked against (the "real gating in the verticals", Phase-2; rule 2 — a cached read
 * model, never a sync call). On apply, the company's entitlement-check Redis cache is invalidated,
 * so a grant promptly OPENS the gate and a revoke promptly CLOSES it.
 *
 ...
 * EntitlementProjectionWriter}. A record arriving WITHOUT a valid {@code id} header is a
 * producer-side contract violation: the listener fails closed (see {@link #eventIdOf}) so the
 * record is DLT'd.
 *

 ...
 * to {@link EntitlementProjectionService}. An immutable record carrying the event id (for
 * idempotency), the owning tenant the consumer binds the write to, the module key (the projection
 * key within the tenant), and the new entitled state ({@code true} for a grant, {@code false} for a
 * revoke).
 *

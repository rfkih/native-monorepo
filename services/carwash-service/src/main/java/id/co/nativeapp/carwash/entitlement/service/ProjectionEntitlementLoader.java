package id.co.nativeapp.carwash.entitlement.service;
...
/**
 * carwash-service's projection-backed {@link EntitlementLoader} — the authoritative fallback the
 ...
 * trusted source — the already-bound request tenant (the record-wash gate runs inside the request
 * scope) or an event's {@code company_id} — and NEVER a client-supplied value. As a fail-fast
 * guard, when a tenant is already bound in {@link TenantContext} this asserts the argument matches
 * it and throws (rather than silently re-binding a different tenant's GUC for the read). When no
 * tenant is bound (an out-of-band cache-seed), the argument is bound as-is.
 */
@Component
public class ProjectionEntitlementLoader implements EntitlementLoader {
  ...
  static final String LOADER_ACTOR = "carwash-entitlement-loader";

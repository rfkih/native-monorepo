...
import id.co.nativeapp.carwash.payment.messaging.PaymentChargeSucceededEvent;
...
import id.co.nativeapp.carwash.ticket.service.TicketCaptureWriter;
 ...
 * PaymentChargeSucceeded} (ADR 0045) to the carwash ticket it settles — idempotently.
 *
 ...
 * CarwashPaymentRepository#findViewByTicketId} read (and the capture writer's own writes) resolve
 * under the right tenant (rule 5). The caller ({@link PaymentChargeSucceededService}) binds the
 * tenant from the event's {@code company_id} before invoking this method. Mirrors {@code
 * entitlement.service.EntitlementProjectionWriter} EXACTLY (carwash's established consumer idiom).
 *
 ...
 * precedent) and the handler returns normally — so the {@code processOnce} claim commits and the
 * event is never retried. Money already moved at the PSP; a human resolves the anomaly. Only an
 * INFRASTRUCTURE failure (e.g. the DB is unreachable) propagates out of this method, rolling back
 * the whole transaction (including the dedupe claim) and letting the container's
 * bounded-retry-then-DLT error handler ({@code config.KafkaConfig}) take over.
 */
@Component
public class PaymentChargeSucceededWriter {
   ...
   */
  static final String VERTICAL = "carwash";

  static final String MISSING_REFERENCE_SOURCE = "carwash.payment-charge.missing-reference";
  static final String UNKNOWN_TICKET_SOURCE = "carwash.payment-charge.unknown-ticket";
  static final String STATE_MISMATCH_SOURCE = "carwash.payment-charge.state-mismatch";
  static final String AMOUNT_MISMATCH_SOURCE = "carwash.payment-charge.amount-mismatch";
  static final String CAPTURE_FAILED_SOURCE = "carwash.payment-charge.capture-failed";
  ...
  private void handle(PaymentChargeSucceededEvent event) {
    if (!VERTICAL.equals(event.vertical())) {
      log.debug(
          "Skipped PaymentChargeSucceeded eventId={} chargeId={} — vertical={} is not carwash",
          event.eventId(),
          event.chargeId(),
          event.vertical());
      return;
    ...
    if (event.referenceId() == null) {
      park(
          ...
          "PaymentChargeSucceeded chargeId="
              + event.chargeId()
              + " carries no reference_id (ticket id) — carwash captures by ticket id and cannot"
              + " proceed");
      return;
    }

    UUID ticketId = event.referenceId();
    Optional<CarwashPaymentView> paymentView = paymentRepository.findViewByTicketId(ticketId);
    ...
    CarwashPaymentView view = paymentView.get();

package id.co.nativeapp.carwash.outletref.service;
 ...
 * user_outlet_assignment_ref} read model (ported verbatim from restaurant-service's {@code
 * outletref} feature).
 *
 * <p>This service is NOT {@code @Transactional} itself — the transactional boundary lives on {@link
 * UserOutletAssignmentRefWriter} (the {@code *Writer} pattern). It binds the tenant from the
...
public class UserOutletAssignmentRefService {
   ...
   *
   * <p>Binds the tenant from the event's {@code company_id} so the writer's RLS-guarded upsert runs
   * in the correct company scope (rule 5 — never trust an inbound header for this).
   *
  ...
  public boolean apply(UserOutletAssignmentEvent event) {
    ...
    try {
      return TenantContext.callAs(
          event.companyId(), "carwash-outlet-consumer", () -> writer.apply(event));

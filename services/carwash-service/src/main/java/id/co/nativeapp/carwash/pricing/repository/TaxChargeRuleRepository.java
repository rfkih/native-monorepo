package id.co.nativeapp.carwash.pricing.repository;
...
/**
 * Spring Data repository for {@link TaxChargeRule} (ported from restaurant-service's {@code
 * pricing} feature). All queries are native (rule 10 / CODE-STRUCTURE.md §3.3). The RLS policy on
 * {@code tax_charge_rule} scopes every query to the bound tenant automatically — no manual {@code
...
public interface TaxChargeRuleRepository extends JpaRepository<TaxChargeRule, java.util.UUID> {
   ...
   *
   * @param ruleKey the rule family key ({@code VAT_CARWASH} or {@code SERVICE_CHARGE})

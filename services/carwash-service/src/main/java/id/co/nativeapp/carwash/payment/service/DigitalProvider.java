 ...
 * verbatim from restaurant-service's {@code payment} feature.
 *
 * <p>In this phase no real PSP adapter exists, so the provider returns a {@link
 * CarwashPayment.Status#PENDING} authorization with {@code providerPending = true} and a
 ...
 *
 * <p>Revenue is recognised only when the future ticket-checkout capture path runs (out of scope
 * here — this phase ports only the provider port + domain pieces): it will transition the payment
 * to {@link CarwashPayment.Status#CAPTURED}. An abandoned digital tender — one that was authorised
 * here but never captured — never produces revenue (ADR 0006 invariant).
 *
...
public class DigitalProvider implements PaymentProvider {
  ...
  /**
   * Returns a {@link CarwashPayment.Status#PENDING} authorization with a placeholder provider
  ...
  public TenderAuthorization authorize(PaymentInstruction instruction) {
    ...
    return new TenderAuthorization(CarwashPayment.Status.PENDING, providerRef, true, null);
  }
}

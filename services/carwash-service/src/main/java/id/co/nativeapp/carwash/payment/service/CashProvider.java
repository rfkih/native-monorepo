...
/**
 * The live cash tender (ADR 0006), ported verbatim from restaurant-service's {@code payment}
 * feature. Cash settles instantly — no external call: it validates the tendered amount covers the
 * amount due, computes the change with {@link Money} (integer minor units; whole rupiah for IDR —
 * never a float), and returns {@link CarwashPayment.Status#CAPTURED}.
 *
...
public class CashProvider implements PaymentProvider {
  ...
  public TenderAuthorization authorize(PaymentInstruction instruction) {
    ...
    return new TenderAuthorization(CarwashPayment.Status.CAPTURED, null, false, change);
  }
}

 ...
 * every authenticated request (ported verbatim from restaurant-service's {@code config} package;
 * see {@code TenantContextHeaderFilter} in the gateway module).
 *
 * <p>This is a thin web-tier adapter that lets {@link
 * id.co.nativeapp.carwash.outletref.service.OutletAccessGuard} — which is not a controller and has
 * no direct access to {@code HttpServletRequest} — consult the caller's roles without threading

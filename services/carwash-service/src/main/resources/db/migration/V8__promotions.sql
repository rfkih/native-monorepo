-- carwash-service V8 — Phase-3 promotions: the per-vertical promo engine (ADR 0026).
--
-- PURPOSE. Three new tables give the carwash POS discount rules beyond the single manual
-- ticket-level discount V6 already added (`carwash_ticket.discount_minor`):
...
-- shipped in restaurant-service (V16), carwash-service (this file), and barbershop-service (V3) —
-- same tables, same columns, same RLS idiom, differing only in the source-aggregate link column name
-- (`ticket_id` here vs `order_id` in restaurant) and the closing ALTER's target table.
--
...
--                                      carwash today (no menu_category-equivalent catalog dimension
--                                      exists here); this service simply never creates a
...
--                                      (`carwash_ticket.discount_minor`, V6) is applied last.
...
-- `carwash_ticket.discount_minor` aggregate the checkout already prices from; `SaleRecorded` keeps
...
-- policy keyed to app.current_tenant, exactly as V1/V4/V5 in this service.
...
CREATE TABLE applied_promotion (
    ...
    -- The carwash_ticket this promotion applied to. Deliberately a PLAIN UUID with NO REFERENCES
    -- clause: carwash_ticket lives in this same database (no cross-service rule-1 concern), but this
    -- table follows the house pattern used elsewhere for non-composition, snapshot-style links (e.g.
    -- carwash_ticket_line.item_id) — applied_promotion is an independent audit trail, not a
    ...
    -- mirrors carwash_ticket.sale_id's own async-capture timing for a digital tender).
    sale_id               UUID         NULL,
    ...
    -- The specific carwash_ticket_line this discount landed on, for a line-scope rule; NULL for a
    -- ticket-level rule.
    line_ref              UUID         NULL,
...
-- ---------------------------------------------------------------------------
-- 4. carwash_ticket.coupon_id — idempotent-replay + receipt display pointer only
-- ---------------------------------------------------------------------------

-- carwash-service V10 — Phase 4 of the POS-parity program (ADR 0027: loyalty-service and
-- eventual-consistency redemption). Extends carwash_ticket's pricing-reconciliation CHECK to account
-- for a points-loyalty redemption, which V9 added the (nullable) column for but did NOT wire into
-- the CHECK — this migration closes that gap.
--
-- ---------------------------------------------------------------------------
...
--         = total_minor
--
-- `loyalty_redeemed_minor` is NULLable (V9): most tickets redeem nothing. COALESCE(...,0) makes the
...
-- re-ADD the V6 form to roll back — no data migration either direction.
--
-- ---------------------------------------------------------------------------
-- What is deliberately NOT in this identity
-- ---------------------------------------------------------------------------
--   * `discount_minor` stays PROMO-ONLY. It is the existing order-level/promotions discount (V8);
...
--   * `gift_card_redeemed_minor` (also added by V9) is DELIBERATELY EXCLUDED from this identity. A
...
-- migration (finance-service V16/V17/V25/V28 do the same for posting_template CHECKs). No RLS/audit
-- change: this migration touches only a CHECK constraint on an existing table.

ALTER TABLE carwash_ticket
    DROP CONSTRAINT ck_carwash_ticket_total_balances;

ALTER TABLE carwash_ticket
    ADD CONSTRAINT ck_carwash_ticket_total_balances

...
/**
 * Externalized configuration for PII column-level encryption (rule 6 — member phone + display name
 * are column-level encrypted, never logged). Ported verbatim from employee-service's {@code
 * PiiEncryptionProperties}.
 *
...
public record PiiEncryptionProperties(@NotBlank String key, @NotBlank String hmacKey) {

  /**
   * Redacted {@code toString} — neither key value must ever reach a log line, a stack trace, or a
  ...
  public String toString() {
    return "PiiEncryptionProperties[key=***REDACTED***, hmacKey=***REDACTED***]";
  }
}

...
/**
 * Kafka consumer wiring for loyalty-service's listeners — {@code SaleRecorded}, {@code
 * GiftCardSold}, {@code SaleVoided}, {@code SaleRefunded} — the same shape entitlement-service /
 * employee-service / finance-service / carwash-service / barbershop-service use.
 *
 ...
 * itself with {@code libs/events AvroSerde} against the SHARED {@code libs/contracts} schema (ADR
 * 0003). We deliberately do NOT use a Confluent kafka-avro-serializer / Schema Registry serde.
 *
...
public class KafkaConfig {
  ...
  /**
   * The typed listener container factory the loyalty {@code @KafkaListener}s bind to: keys are
   ...
   * record survives a rolled-back business transaction). Mirrors the wiring in barbershop-service's
   * {@code KafkaConfig}.
   */
  @Bean
   ...
   * — overriding the auto-configured template's String/String defaults.
   */
  @Bean

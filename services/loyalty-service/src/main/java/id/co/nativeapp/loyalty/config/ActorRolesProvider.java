 ...
 * every authenticated request (ported verbatim from barbershop-service's {@code
 * ActorRolesProvider}; see {@code TenantContextHeaderFilter} in the gateway module).
 *
 * <p>This is a thin web-tier adapter that lets {@link
 * id.co.nativeapp.loyalty.earnrule.service.EarnRuleService EarnRuleService} — which is not a
 ...
 * copy before injecting its own (see {@code TenantContextHeaderFilter} in the gateway module).
 * Either way the header is trusted at this point.
 *
 ...
 * roles are present and the caller is treated as unauthenticated (safe default for the guard).
 */
@Component

 ...
 * only the specific actuator endpoints a probe/scrape needs — {@code /actuator/health/**}, {@code
 * /actuator/info}, {@code /actuator/prometheus} — are {@code permitAll} so they answer without a
 * token. The whole {@code /actuator/**} tree is deliberately NOT opened: widening {@code
 * management.endpoints.web.exposure.include} (e.g. to {@code env}/{@code heapdump}/{@code
 * threaddump}) must never silently expose those unauthenticated at the edge. Everything else

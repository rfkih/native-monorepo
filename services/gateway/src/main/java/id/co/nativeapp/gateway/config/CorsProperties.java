 ...
 * the pre-0051 edge). The bundled build is served from a local origin ({@code https://localhost})
 * and calls the API cross-origin, so THAT deploy sets {@code NATIVE_GATEWAY_CORS_ALLOWED_ORIGINS}
 * to its exact origin(s).
 *
 * <p>It is an <strong>exact</strong> allow-list, never a wildcard: the gateway is the security
 * edge, and {@code *} would let any web origin drive the authenticated API from a victim's browser.
...
public record CorsProperties(List<String> allowedOrigins) {
  public CorsProperties {
    ...
    // `*` (with credentials off it is not the classic foot-gun, but it is still far broader than
    // any
    // bundled shell needs) or a cleartext non-loopback origin fails the app fast at startup, the
    // same
    // way a missing route target does (ENGINEERING-STANDARDS §7).
    for (String origin : allowedOrigins) {
      if (origin == null || origin.isBlank() || origin.contains("*")) {
        throw new IllegalArgumentException(
            "native.gateway.cors.allowed-origins must be exact origins, never a wildcard: "
                + origin);
      }

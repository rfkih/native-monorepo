 ...
 *
 * <p>The end-to-end tests publish a {@code ConsolidationClosed} Avro message (bytes built with
 * {@code libs/events AvroSerde}) onto the topic; the notification {@code @KafkaListener} consumes
 * it, creates + delivers a notification, persists a {@code delivery_receipt}, and writes a {@code
 * DeliveryReceipt} outbox row. Using a genuine broker (not an embedded mock) exercises the real
 * transport: raw bytes on the wire, the byte[] value deserializer, and the container's offset/ack

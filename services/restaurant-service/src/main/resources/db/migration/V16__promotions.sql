-- restaurant-service V16 — Phase-3 promotions: the per-vertical promo engine (ADR 0026).
--
-- PURPOSE. Three new tables give the restaurant POS discount rules beyond the single manual
-- order-level discount V8 already added (`restaurant_order.discount_minor`):
--   1. promo_rule         — the configured catalog of discount rules (percent/amount off the whole
--                            order, percent off a line/category, happy-hour windows, coupon-gated).
...
--                            order, so later edits to promo_rule (rate change, retirement) never
...
-- shipped in restaurant-service (this file), carwash-service (V8), and barbershop-service (V3) —
-- same tables, same columns, same RLS idiom, differing only in the source-aggregate link column name
-- (`order_id` here vs `ticket_id` in carwash/barbershop) and the closing ALTER's target table.
--
...
--                                      matching line (ITEM or CATEGORY scope) before any order-level
--                                      rule is considered.
--   2. AUTOMATIC order-level rules   — PERCENT_OFF_ORDER / AMOUNT_OFF_ORDER rules that do NOT require
...
--   4. MANUAL discount last          — the pre-existing staff-entered order-level discount
--                                      (`restaurant_order.discount_minor`, V8) is applied last.
--   5. CLAMP                         — the total of every layer above is clamped so the sum never
--                                      exceeds the order subtotal (a discount can zero out a sale, it
--                                      can never make it negative).
--
-- FINANCE IS UNCHANGED. Every layer above collapses into the SAME pre-existing
-- `restaurant_order.discount_minor` aggregate the checkout already prices from; `SaleRecorded` keeps
...
-- policy keyed to app.current_tenant, exactly as V1/V2/V5/V6 in this service.
...
CREATE TABLE promo_rule (
    ...
    -- (menu_category, V6) — verticals without categories (carwash, barbershop) simply never create
    -- a CATEGORY-scoped rule; scope_ref_id is a deliberately opaque UUID (not a FK — no
    ...
    -- Minimum order subtotal (minor units, order currency) required for this rule to qualify.
...
-- tax_charge_rule resolution index (V5) in this same service.
...
CREATE TABLE applied_promotion (
    ...
    -- The restaurant_order this promotion applied to. Deliberately a PLAIN UUID with NO REFERENCES
    -- clause: restaurant_order lives in this same database (no cross-service rule-1 concern), but
    -- this table follows the house pattern used elsewhere for non-composition, snapshot-style links
    -- (e.g. carwash_ticket_line.item_id) — applied_promotion is an independent audit trail, not a
    -- CASCADE-owned child of the order.
    order_id              UUID         NOT NULL,

    -- The sale this promotion's discount ultimately rode on (NULL until the order captures a sale —
    -- mirrors restaurant_order.sale_id's own async-capture timing for a digital tender).
    sale_id               UUID         NULL,
    ...
    -- The specific order_line this discount landed on, for a line-scope rule; NULL for an
    -- order-level rule.
    line_ref              UUID         NULL,

    -- Money (libs/money) = integer minor units + ISO-4217 code. NEVER a float (rule 8). The actual
    -- minor-unit amount THIS rule discounted off THIS order, already clamped per the composition
...
-- and the promotions applied to a given order (recompute / receipt rendering before a sale exists).
CREATE INDEX idx_applied_promotion_sale ON applied_promotion (company_id, sale_id);
CREATE INDEX idx_applied_promotion_order ON applied_promotion (company_id, order_id);

-- ---------------------------------------------------------------------------
-- 4. restaurant_order.coupon_id — idempotent-replay + receipt display pointer only
-- ---------------------------------------------------------------------------
...
-- without a join. Nullable: most orders never use a coupon.

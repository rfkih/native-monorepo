-- restaurant-service V17 — Phase 4 of the POS-parity program (ADR 0027: loyalty-service and
...
--
-- ---------------------------------------------------------------------------
-- General notes (mirrors V14 user_outlet_assignment_ref precedent)
-- ---------------------------------------------------------------------------
...
--     CHECK), identical predicate to every other restaurant-service table.
...
-- is inserted. Same "checkout-time snapshot, not a join target" reasoning as e.g.
...
-- ---------------------------------------------------------------------------
-- restaurant_order — nullable, additive checkout columns for a loyalty/gift-card redemption
-- ---------------------------------------------------------------------------
-- Selected at checkout (before the tender — and, for a digital tender, before capture — is known).
-- PaymentCaptureWriter loads the Order to reconstruct pricing for a deferred digital-tender capture
-- (see PaymentCaptureWriter#reconstructBreakdown); `sale` itself persists NO breakdown-shaped columns
-- (id/business_id/amount/occurred_at/idempotency_key + Auditable only — see Sale.java), so these
-- redemption selections must live on restaurant_order, the aggregate BOTH the immediate cash-checkout
-- SaleWriter.recordInCurrentTx call AND the deferred PaymentCaptureWriter.capture call have available
-- at SaleRecorded-emission time. All five are nullable: most orders redeem nothing.

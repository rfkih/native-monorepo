 ...
 * id.co.nativeapp.restaurant.entitlement.domain.EntitlementProjection}) — the projection {@code
 * selforder.service.SelfOrderService} checks every self-order-create against (Phase 6, ADR 0029;
 * rule 2 — a cached read model, never a sync call). On apply, the company's entitlement-check Redis
 * cache is invalidated, so a grant promptly OPENS the gate and a revoke promptly CLOSES it.
 *
 ...
 * durable event id (not the offset) survives rebalances and compacted replay; the dedupe +
 * projection upsert happen transactionally in {@link EntitlementProjectionService} / {@link
 * id.co.nativeapp.restaurant.entitlement.service.EntitlementProjectionWriter}. A record arriving
 ...
 * partition (see {@link id.co.nativeapp.restaurant.config.KafkaConfig}).
 */
@Component
public class EntitlementEventListener {
  ...
  private static EntitlementProjectedEvent decode(
    ...
    try {
      ...
      throw new EntitlementDecodeException(
   ...
   * is a contract violation: we FAIL CLOSED — throwing {@link EntitlementMissingEventIdException},
  ...
  private static UUID eventIdOf(ConsumerRecord<String, byte[]> record) {
    ...
    if (header == null || header.value() == null) {
      throw new EntitlementMissingEventIdException(

...
/**
 * Loads the {@code SaleRecorded} Avro schema from the classpath
 * ({@code avro/SaleRecorded.avsc}) and builds {@link GenericRecord}s from it — no
 * Avro code-generation plugin, exactly as M1.4 specifies. The schema is the
 * single source of truth registered in {@code docs/EVENT-CATALOG.md}; this class
 * just parses it once and projects a {@link Sale} onto it.
 *
 * <p>The record is serialized to the outbox payload via {@code libs/events}
 * {@link id.co.nativeapp.events.AvroSerde AvroSerde}. Field shape matches
 * ARCHITECTURE.md §5: {@code sale_id}, {@code company_id}, {@code business_id}
 * (strings), {@code amount_minor} (long), {@code currency} (string), and
 * {@code occurred_at} (long, logicalType {@code timestamp-millis}).
 */
public final class SaleRecordedSchema {
    ...
    /**
     * Builds a {@code SaleRecorded} {@link GenericRecord} from a persisted sale.
     * The amount is taken from the sale's {@link Money} (integer minor units +
    ...
    private static Schema parse() {
        try (InputStream in =
                SaleRecordedSchema.class.getClassLoader().getResourceAsStream(RESOURCE)) {
            if (in == null) {

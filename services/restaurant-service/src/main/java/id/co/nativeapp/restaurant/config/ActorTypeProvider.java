 ...
 * sale.service.SaleWriter}, {@code payment.service.PaymentWriter}) consult the caller's actor type
 * without threading it as a parameter through the entire call chain or changing the shared {@code
 ...
 * guard inert on the async {@code PaymentChargeSucceeded} capture path ({@code
 * PaymentCaptureWriter#capture} runs on a Kafka consumer thread — there is no HTTP request, so this
 * always resolves {@code "user"} there, and the guard never fires) and inert for a normal {@code
 * actor_type=user} login that carries no operator session (today's exact pre-P4 behaviour).
 */
@Component
public class ActorTypeProvider {
  ...
  public String currentActorType() {
    try {
      ...
      // safe default: "user" (see class javadoc — this is what keeps the async capture path inert).
    }
    return USER;
  }

 ...
 * CatalogAdvice} of its own (that class lives in carwash), so this mirrors ITS style: narrow, feature-
 * scoped handlers ordered ahead of the shared {@code libs/security ApiExceptionHandler} catch-all.
 *
 * <ul>
 ...
 *       rule is not currently effective, supplied at checkout/pay time → {@code 422} ({@code
 *       coupon-invalid}).
...
public class PromotionAdvice {
  ...
  public ProblemDetail handleConflict(DataIntegrityViolationException ex, HttpServletRequest request) {
    ...
    problem.setDetail("The operation conflicted with an existing row (e.g. a duplicate coupon code).");
    return problem;
  }

 ...
 *
 * <p><strong>{@code kid} / secret generation.</strong> {@code kid} is 8 random bytes hex-encoded
 * (16 characters, fitting the {@code VARCHAR(16)} column exactly) — it is a compact correlation id,
 * not itself a secret. The secret is 32 random bytes (AES-256/HMAC-SHA256-key-length),
 * base64-encoded before being handed to the entity, which encrypts it at rest via the {@code

 ...
 * must NEVER be active in production. It is gated to register only under the {@code dev} profile OR
 * when {@code native.dev-tenant-filter.enabled=true} is set explicitly, so a production profile
 ...
 * sale.company_id} column is {@code VARCHAR(64)}, but the {@code outbox} {@code company_id} column
 * is {@code UUID}; a non-UUID tenant id would persist the sale and then blow up on the outbox write
 * ({@code UUID.fromString}) — a {@code 500} after a partial effect. So this filter validates the

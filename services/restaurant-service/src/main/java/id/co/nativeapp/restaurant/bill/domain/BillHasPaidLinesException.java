 ...
 * paid split-check). Cancelling would strand the recorded sales — the remainder must be settled (or
 * the paid checks reversed via the return-sale flow) first. Applies to EVERY role. Maps to {@code
 * 409 Conflict} via {@link id.co.nativeapp.restaurant.bill.controller.BillExceptionHandler}.
 */

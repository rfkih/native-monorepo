 ...
 * for one checkout/pay/quote, and nothing it can mutate (pure evaluation, no writes).
 *
 ...
 *     parked order re-evaluates at pay time); determines the effective rule window and the
 *     happy-hour day/time gate in each rule's own timezone
 ...
 *     before lookup
 * @param manualDiscountMinor the pre-existing staff-entered order/check-level discount, applied
 *     LAST in the composition order; must be &ge; 0
 */

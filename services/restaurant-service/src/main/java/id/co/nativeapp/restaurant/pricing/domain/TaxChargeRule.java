...
/**
 * A VERSIONED + EFFECTIVE-DATED tax/service-charge configuration rule for the POS checkout pricing
 * pipeline (Phase 2 — pricing: tax + service charge + discount).
 *
 * <p>Mirrors the {@code statutory_rule} pattern from {@code employee-service}: the ALGORITHM lives
 * in Java ({@code OrderWriter.checkout}); every FIGURE (rates, flags) is data here. Swapping
 ...
 *
 * <ul>
 *   <li>{@code PB1_RESTAURANT} — the restaurant-sale tax rate in basis points (1000 bp = 10%).
 *       Whether this is PB1, PPN, or a combined rate is <strong>SME-gated</strong>; the seeded
 *       value of 1000 bp (10%) is ILLUSTRATIVE only.
 *   <li>{@code SERVICE_CHARGE} — the service-charge rate in basis points (500 bp = 5%). Whether
 *       service charge is revenue or a liability (tip pool) is <strong>SME-gated</strong>.
 * </ul>
 *
...
public class TaxChargeRule extends Auditable {
  ...
  /** Rule key for the restaurant-sale tax (PB1 / PPN — regime is SME-gated). */
  public static final String KEY_PB1 = "PB1_RESTAURANT";
  ...
  /**
   * The rule family key: {@code PB1_RESTAURANT} or {@code SERVICE_CHARGE}. New rule types may be

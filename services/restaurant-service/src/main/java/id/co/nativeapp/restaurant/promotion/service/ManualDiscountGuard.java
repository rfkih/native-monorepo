 ...
 * ({@code OrderWriter.checkout}/{@code payParked}, {@code BillWriter.payBill}) so the guard cannot be
 * bypassed via one path while enforced on another (the {@code OutletAccessGuard} sharing pattern).
 *
 ...
 * service-layer test, where the {@code OutletAccessGuard} grandfather clause applies the same trust.
 */
@Component

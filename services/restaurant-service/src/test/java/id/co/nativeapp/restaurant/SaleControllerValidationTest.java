 ...
 *
 * <p>Bean-validation failures on the {@code @Valid} body and a domain
 * {@link IllegalArgumentException} (e.g. an unknown ISO-4217 currency from
 * {@code libs/money}) are mapped to {@code 400} by {@link ApiExceptionHandler}, while a
 * valid request still reaches the service. The idempotent-retry path is a {@code 200}
 * returned by {@link SaleController} and is unaffected by the advice.
 */
...
class SaleControllerValidationTest {
    ...
    void blankCurrencyIsRejectedWith400() throws Exception {
        String body = """
                ...
                """;
        mockMvc.perform(post("/sales").contentType(MediaType.APPLICATION_JSON).content(body))

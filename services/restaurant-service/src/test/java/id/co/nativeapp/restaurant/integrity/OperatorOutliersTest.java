...
class OperatorOutliersTest {
  ...
  void anOperatorVoidingFarMoreThanTheirColleaguesIsFlagged() {
    List<OperatorRate> rates =
        List.of(
            OperatorRate.counting("budi", 20, 100, 400_000L), // 20%
            OperatorRate.counting("sari", 2, 100, 40_000L), //  2%
            OperatorRate.counting("dewi", 3, 100, 60_000L)); //  3%
  ...
  void theBaselineExcludesTheOperatorBeingJudged() {
    ...
    List<OperatorRate> rates =
        List.of(
            OperatorRate.counting("budi", 30, 100, 600_000L), //
            OperatorRate.counting("sari", 10, 100, 200_000L));
  ...
  void whenMostOfTheRosterBehavesTheSameWayNobodyIsAnOutlier() {
    ...
    List<OperatorRate> rates =
        List.of(
            OperatorRate.counting("budi", 20, 100, 400_000L),
            OperatorRate.counting("sari", 20, 100, 400_000L),
            OperatorRate.counting("dewi", 2, 100, 40_000L));
  ...
  void aLoneOperatorIsNeverAnOutlier() {
    ...
    List<OperatorRate> rates = List.of(OperatorRate.counting("owner", 50, 60, 1_000_000L));
  ...
  void aTinySampleIsIgnoredHoweverExtremeItsRate() {
    ...
    List<OperatorRate> rates =
        List.of(
            OperatorRate.counting("baru", 3, 3, 60_000L), //
            OperatorRate.counting("lama", 4, 400, 80_000L));
  ...
  void anOperatorWithNoOpportunityHasNoRate() {
    List<OperatorRate> rates =
        List.of(
            OperatorRate.counting("nol", 9, 0, 0L), // refunded old sales, took none this window
            OperatorRate.counting("aktif", 1, 100, 20_000L));
  ...
  void anOperatorIsFlaggedWhenNobodyElseDidItAtAll() {
    ...
    List<OperatorRate> rates =
        List.of(
            OperatorRate.counting("budi", 6, 100, 120_000L), //
            OperatorRate.counting("sari", 0, 100, 0L));
  ...
  void beingMerelyAboveAverageIsNotEnough() {
    ...
    List<OperatorRate> rates =
        List.of(
            OperatorRate.counting("budi", 6, 100, 120_000L), //
            OperatorRate.counting("sari", 5, 100, 100_000L));
  ...
  void theBoundaryIsInclusiveSoExactlyTwiceCounts() {
    ...
    List<OperatorRate> rates =
        List.of(
            OperatorRate.counting("budi", 10, 100, 200_000L), //
            OperatorRate.counting("sari", 5, 100, 100_000L));
  ...
  void flaggedOperatorsAreOrderedByTheMoneyInvolvedThenDeterministically() {
    List<OperatorRate> rates =
        List.of(
            OperatorRate.counting("kecil", 10, 100, 50_000L),
            OperatorRate.counting("besar", 10, 100, 900_000L),
            OperatorRate.counting("sama", 10, 100, 50_000L),
            OperatorRate.counting("bersih", 0, 400, 0L));
  ...
  @Test
  void aMoneyValuedMeasureIsGatedOnHowOftenItHappenedNotOnHowMuchItCameTo() {
    // The discount check's numerator is an AMOUNT. Gating on the numerator would make the floor a
    // one-rupiah floor, and since the rest of the outlet discounted nothing, `mine >= 0` is
    // trivially true — a single small discount would put a name in front of the owner. The event
    // count is what the guard has to read.
    List<OperatorRate> oneSmallDiscount =
        List.of(
            new OperatorRate("budi", 1_000L, 500_000L, 1_000L, 1),
            OperatorRate.counting("sari", 0, 500_000L, 0L));

    assertThat(OperatorOutliers.outliers(oneSmallDiscount, FACTOR, MIN_COUNT)).isEmpty();

    // The same operator, discounting habitually rather than once, is exactly what the signal is
    // for.
    List<OperatorRate> habitual =
        List.of(
            new OperatorRate("budi", 90_000L, 500_000L, 90_000L, 9),
            OperatorRate.counting("sari", 0, 500_000L, 0L));

    assertThat(OperatorOutliers.outliers(habitual, FACTOR, MIN_COUNT))
        .extracting(OperatorRate::actor)
        .containsExactly("budi");
  }
  ...
  void totalValueSumsTheMoneyBehindTheFlaggedOperators() {
    assertThat(
            OperatorOutliers.totalValue(
                List.of(
                    OperatorRate.counting("a", 1, 2, 30_000L),
                    OperatorRate.counting("b", 1, 2, 12_500L))))
        .isEqualTo(42_500L);

 ...
 * owns the migrated {@code sale} table; the baseline migration's
 ...
 * in a static initializer and deliberately never stopped per-class. Spring caches
 * a {@code @SpringBootTest} context keyed by its configuration; since the test
 * classes share this identical configuration, the cached context (and its Hikari
 * pool) is reused across them — only safe if the underlying container outlives
 * every class. Testcontainers' Ryuk shutdown hook reaps it at JVM exit.
 */
abstract class PostgresRlsTestBase {
    ...
    /**
     * Start every test from empty {@code sale} + {@code outbox} tables so the
     ...
     * tables may not yet exist on the very first invocation before Flyway has run,

...
/**
 * The headline feature under CONCURRENCY: two threads record a sale at the same time with the SAME
 * {@code idempotency_key} in the SAME tenant scope.
 *
 * <p>Proves the concurrency-safe idempotency contract of {@link SaleService}:
 *
 * <ul>
 *   <li>exactly ONE {@code sale} row exists (counted over the admin/BYPASSRLS connection, since
 *       {@code sale} is {@code FORCE} RLS);
 ...
 *       one {@code created=true} and one {@code created=false}, both resolving to the same sale id.
 * </ul>
 *
 ...
 * returns the winner's sale with {@code created=false}.
 */
@SpringBootTest
class RecordSaleConcurrencyTest extends PostgresRlsTestBase {
  ...
  void concurrentSameKeyYieldsOneSaleOneEventAndTwoSuccessfulResults() throws Exception {
    ...
    Callable<RecordSaleResult> attempt =
        () ->

 ...
 *
 * <p>A sale recorded under tenant A is invisible to tenant B. The read path
 * ({@link SaleService#findAllForCurrentTenant()}) carries NO {@code WHERE
 * company_id} and never calls the synchronizer by hand — only
 * {@link RlsAutoApplyAspect} sets the tenant GUC on each {@code @Transactional}
 * unit of work. This models a developer who forgets the tenant and proves they are
 * still protected (rule 5).
 *
 * <p>Runs as the unprivileged {@code app_user} role (see {@link PostgresRlsTestBase});
 * {@code FORCE ROW LEVEL SECURITY} in the baseline binds even that owning role.
 */
@SpringBootTest
class SaleTenancyIsolationTest extends PostgresRlsTestBase {
    ...
    void aSaleRecordedUnderTenantAIsInvisibleToTenantB() throws Exception {
        ...
        UUID saleA = TenantContext.callAs(TENANT_A, ACTOR_A, () ->
                saleService.recordSale(new RecordSaleCommand(
                        businessId, 1_000_000L, "IDR", occurredAt, "a-key")).sale().getId());
        UUID saleB = TenantContext.callAs(TENANT_B, ACTOR_B, () ->
                saleService.recordSale(new RecordSaleCommand(
                        businessId, 2_000_000L, "IDR", occurredAt, "b-key")).sale().getId());

        // Inside A's scope: only A's sale is visible. No WHERE clause anywhere; RLS only.
        List<UUID> aVisible = TenantContext.callAs(TENANT_A, ACTOR_A, () ->
        ...
        List<UUID> bVisible = TenantContext.callAs(TENANT_B, ACTOR_B, () ->
                saleService.findAllForCurrentTenant().stream().map(Sale::getId).toList());

 ...
 *
 * <p>Proves the {@code SaleRecorded.avsc} on the classpath parses, that a
 * {@link GenericRecord} built from it round-trips through {@code libs/events}
 * {@link AvroSerde}, and that the backward-compatibility gate accepts the schema
 * against itself and against an added-optional-field variant, while rejecting an
 * incompatible change (a new required field with no default). CLAUDE.md rule 7:
 * event schema changes are backward-compatible ONLY.
 */
class SaleRecordedContractTest {
    ...
    void avscParsesFromClasspath() {
        ...
        assertThat(schema.getFullName())
                .isEqualTo("id.co.nativeapp.events.restaurant.SaleRecorded");
        ...
        assertThat(schema.getField("amount_minor").schema().getType())
        ...
        assertThat(schema.getField("occurred_at").schema().getLogicalType())
                .isNotNull();
    ...
    void genericRecordRoundTripsThroughAvroSerde() {
        ...
        assertThat(decoded.get("sale_id").toString())
    ...
    void addingARequiredFieldWithoutDefaultBreaksBackwardCompatibility() {
        Schema v1 = SaleRecordedSchema.schema();
        Schema incompatible = new Schema.Parser().parse(
                """
                {
                  "type": "record",
                  "name": "SaleRecorded",

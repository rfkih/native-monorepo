 ...
 *
 * <p>Recording a sale (in a {@link TenantContext} scope) persists the sale and
 * writes EXACTLY ONE {@code SaleRecorded} row to the outbox, atomically. The
 * payload deserializes (via {@code libs/events} {@link AvroSerde}) to the right
 * {@code sale_id} / {@code amount_minor} / {@code currency}. Submitting again with
 * the SAME {@code idempotency_key} returns the existing sale and the outbox STILL
 * holds exactly one {@code SaleRecorded} (idempotency on retry).
 *
 * <p>The full context boots, so the auto-RLS aspect is active, Flyway has run, and
 * Hibernate {@code ddl-auto=validate} has verified the mapping against the schema.
 */
@SpringBootTest
class RecordSaleAcceptanceTest extends PostgresRlsTestBase {
    ...
    void recordingASaleWritesExactlyOneSaleRecordedAndIsIdempotentOnRetry() throws Exception {
        ...
        RecordSaleResult first = TenantContext.callAs(TENANT_A, ACTOR_A, () ->
                saleService.recordSale(command));
        ...
        RecordSaleResult retry = TenantContext.callAs(TENANT_A, ACTOR_A, () ->
                saleService.recordSale(command));
    ...
    private long saleRowCountAsAdmin() throws Exception {
        try (java.sql.Connection admin = java.sql.DriverManager.getConnection(

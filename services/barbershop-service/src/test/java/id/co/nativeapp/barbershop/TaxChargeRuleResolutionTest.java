 ...
 * scenario seeds its rows directly over the admin/BYPASSRLS connection, mirroring exactly how the
 * V1 baseline migration itself seeds the illustrative {@code VAT_BARBERSHOP} row. Every scenario
 * uses its OWN dedicated tenant id: {@code tax_charge_rule} is deliberately NOT truncated by {@link
 * PostgresRlsTestBase#resetTables()} (so the V1 migration seed survives across the whole suite —
 * see that method's javadoc), so tests must not collide on a shared tenant/rule_version.
 *
 ...
 * already uses in this module, so the auto-RLS aspect sets the tenant GUC the repository read
 * relies on (rule 5).
 */
@SpringBootTest
class TaxChargeRuleResolutionTest extends PostgresRlsTestBase {
  ...
  void noServiceChargeRuleFallsThroughToZero() throws Exception {
    ...
    // Only VAT_BARBERSHOP is seeded — no SERVICE_CHARGE row for this tenant, mirroring the
    // barbershop V1 baseline seed (see that migration's note 2).
    seedRule(
        tenant,
        TaxChargeRule.KEY_VAT,
        "TEST-NOSC",
  ...
  // -----------------------------------------------------------------------
  // 4. Resolution is keyed on VAT_BARBERSHOP, not carwash's VAT_CARWASH
  // -----------------------------------------------------------------------

  @Test
  void keyConstantIsVatBarbershopNotCarwashsVatCarwashKey() {
    assertThat(TaxChargeRule.KEY_VAT).isEqualTo("VAT_BARBERSHOP");
  }

  @Test
  void aRowSeededUnderCarwashsOldVatCarwashKeyIsNeverResolved() throws Exception {
    String tenant = "77777777-7777-7777-7777-770000000004";
    // Simulate a stray/leftover row under the OLD carwash key — the barbershop resolver must NOT
    // pick it up; it queries TaxChargeRule.KEY_VAT ("VAT_BARBERSHOP") only.
    seedRule(
        tenant,
        "VAT_CARWASH",
        "TEST-WRONG-KEY",
  ...
  // -----------------------------------------------------------------------
  // 5. No rule seeded at all → full zero fall-through (mirrors carwash's parity test)
  // -----------------------------------------------------------------------
  ...
  // -----------------------------------------------------------------------
  // Admin seeding helper (bypasses RLS — a BYPASSRLS superuser connection, same pattern the V1
  // migration itself uses, and the same admin-connection pattern this test module already uses for
  // post-condition assertions elsewhere).
  // -----------------------------------------------------------------------

...
/**
 * Consumer-side acceptance tests for the {@code UserOutletAssignmentChanged} listener, ported from
 * carwash-service's {@code UserOutletAssignmentConsumerAcceptanceTest} into barbershop-service's
 * top-level test package (matching this service's flat test-class convention).
 *
...
class UserOutletAssignmentConsumerAcceptanceTest extends PostgresRlsTestBase {
  ...
  private static final String USER_ATTENDANT = "kc-attendant-01";
  ...
  void assignedEventUpsertsActiveRow() {
    ...
    assertThat(countActiveRowsAdmin(COMPANY_A, USER_ATTENDANT, ORG_UNIT_OUTLET)).isOne();
  }
  ...
  void unassignedEventUpsertsInactiveRow() {
    ...
    assertThat(countActiveRowsAdmin(COMPANY_A, USER_ATTENDANT, ORG_UNIT_OUTLET)).isZero();

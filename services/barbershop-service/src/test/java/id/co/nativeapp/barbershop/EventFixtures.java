...
import id.co.nativeapp.barbershop.payment.messaging.PaymentChargeSucceededListener;
...
final class EventFixtures {
  ...
  // ---- PaymentChargeSucceeded (ADR 0045, against the barbershop consumer-copy schema) —
  // published by payment-service in production; these fixtures publish the identical wire shape
  // (raw Avro bytes + the durable "id" header) so the real @KafkaListener consumes them exactly as

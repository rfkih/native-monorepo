 ...
 * feature's LAYER SUB-PACKAGES. Copied from carwash-service's {@code LayeredArchitectureTest} with
 * the package renamed (ADR 0024).
 *
 * <p>Each feature package {@code id.co.nativeapp.barbershop.<feature>} holds its classes in a layer
 ...
 *
 * <p>The {@code Projection} layer is a required (non-optional) layer because barbershop-service
 * carries a projection package under {@code catalog.projection}/{@code ticket.projection}/{@code
 * pricing.projection} for the native read queries (CODE-STRUCTURE §3.3). The {@code
 * repositoryQueriesAreNative} rule fails the build on any JPQL {@code @Query}.
 */
class LayeredArchitectureTest {

  private static final String BASE_PACKAGE = "id.co.nativeapp.barbershop";
   ...
   * domain, with dto as the boundary-translation layer, projection as the native-query read-model
   * layer, and messaging as the feature-local event plumbing. {@code
   * consideringOnlyDependenciesInLayers()} scopes the check to dependencies whose target is one of
   * these layers, so the cross-cutting {@code config} package (the
   * {@code @Configuration}/filter/advice set, incl. the shared decode/missing-event-id exceptions)
   * and the shared libs are out of scope.
   */
  @Test
  void featureLayersRespectTheLayeredArchitecture() {
    ArchRule rule =
        layeredArchitecture()
            ...
            .layer("Dto")
            .definedBy("..dto..")
            // Projection holds native-query read models produced by the repository layer and
            // consumed (mapped to a dto) by the service layer — reachable from neither the
            // controller nor below. CODE-STRUCTURE §3.3.
            .layer("Projection")
   ...
   * other event-driven integration in this fleet) exists to avoid. Barbershop does not yet consume
   * the operator token (deferred to P4), but the guard applies fleet-wide from P2 so no future
   * feature introduces the forbidden dependency ahead of that wiring.
   */
  @Test

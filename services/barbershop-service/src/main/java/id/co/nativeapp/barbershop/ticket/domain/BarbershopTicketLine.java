...
/**
 * One priced line on a {@link BarbershopTicket}: a snapshot of a catalog item's name and unit price
 * at checkout time (V1 baseline), so a receipt stays reproducible even if the catalog row changes
 * later. Renamed from carwash-service's {@code CarwashTicketLine}.
 *
 ...
 * by the {@code barbershop_ticket_line} RLS policy (rule 5).
 */
@Entity
@Table(name = "barbershop_ticket_line")
public class BarbershopTicketLine extends Auditable {
  ...
  protected BarbershopTicketLine() {
    // for JPA
  }

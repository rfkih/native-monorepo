 ...
 * {@code ruleType} cannot be changed via PATCH (re-create instead); the type/parameter cross-checks
 * in {@link PromoRuleCreateRequest}'s javadoc are re-validated against the resulting combination
 * when a parameter that participates in them is supplied. Ported verbatim from restaurant-service
 * via carwash-service.
 *

package id.co.nativeapp.barbershop.ticket.controller;
...
/**
 * {@code /api/v1/barbershop/tickets} — the barbershop POS checkout (ADR 0024, vertical path
 * prefixing per ADR 0023). The tenant ({@code company_id}) and actor come from the bound {@link
 * id.co.nativeapp.tenant.TenantContext TenantContext}, never the request body (rule 5).
 *
 * <p>Every write ({@code quote}/{@code checkout}/{@code capture}) is gated by the barbershop
 ...
 * config.OutletAccessAdvice}), and requires a barber ({@code staffProfileId}, {@code 400} when
 * missing — every cut has a barber).
 */
@Tag(
    name = "Barbershop Tickets",
    description = "Quote, checkout, capture, and retrieve barbershop tickets")
@RestController
@RequestMapping("/api/v1/barbershop/tickets")
public class TicketController {
  ...
  @Operation(
      ...
      description =
          "Stateless price preview for the requested lines — resolves + validates catalog items and"
              ...
              + " emitted. Rejected with 403 when the company is not entitled to the barbershop"
              + " module.")
  @PostMapping("/quote")
  ...
  @Operation(
      ...
      description =
          "Records a barbershop ticket: resolves + prices the requested lines, authorizes the"
              + " tender, and — for CASH — recognises revenue immediately (one SaleRecorded + the"
              + " declared metric set, atomically). A digital tender (QRIS/CARD) persists PENDING;"
              + " revenue is recognised only when POST /{id}/capture runs. staffProfileId is"
              + " MANDATORY — every cut has a barber; missing it is rejected with 400. Returns 201"
              + " Created with a Location header on the first write; returns 200 OK on a retry"
              + " carrying the same idempotencyKey (no second event). Rejected with 403 when the"
  ...
  public ResponseEntity<TicketResponse> checkout(@Valid @RequestBody CheckoutRequest request) {
    ...
    return result.created()
        ? ResponseEntity.created(URI.create("/api/v1/barbershop/tickets/" + body.ticketId()))
            .body(body)
  ...
  @Operation(
      ...
      description =
          "Captures a PENDING QRIS/CARD tender on a ticket: transitions the payment to CAPTURED,"
              + " links the sale, and recognises revenue (SaleRecorded + metrics). Idempotent:"
              + " capturing an already-CAPTURED payment returns 200 with no second event. Rejected"
              + " with 403 when the company is not entitled to the barbershop module, 404 if the"
              + " ticket is unknown, and 409 if the payment is not in a capturable state.")

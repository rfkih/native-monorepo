package id.co.nativeapp.barbershop.entitlement.repository;
 ...
 * — tenant scoping comes solely from the auto-applied RLS GUC (rule 5). The gate looks a module up
 * by key <em>within</em> the bound tenant to learn whether the company is entitled; RLS makes a
 * cross-tenant projection row invisible (empty), so a company can never be entitled via another
 * tenant's row.
 */

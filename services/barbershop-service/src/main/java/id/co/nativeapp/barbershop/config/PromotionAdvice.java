 ...
 * PromotionAdvice} via carwash-service's identical port).
 *
 * <ul>
 ...
 * handler service-wide (409, generic "catalog-item-conflict" detail); declaring a SECOND handler for
 * the identical exception type here — with neither advice bean carrying an {@code @Order} — would
 * make Spring's cross-advice {@code @ExceptionHandler} resolution ambiguous/bean-order-dependent for
 * EVERY {@code DataIntegrityViolationException} in this service, not just a duplicate coupon. Left
 * unhandled here on purpose: a duplicate coupon code still surfaces as {@code 409 Conflict} via
 * {@link CatalogAdvice}'s existing catch-all (a slightly generic {@code catalog-item-conflict}
 * detail message rather than a promotion-specific one) — no ambiguity, no raw {@code 500}. Flagged
 * for the tech lead/integration-engineer as a follow-up (either give {@code CatalogAdvice} a
 * coupon-aware detail branch, or promote both handlers behind a single ordered advice).
 */

...
/**
 * A VERSIONED + EFFECTIVE-DATED tax/service-charge configuration rule for the barbershop POS
 * checkout pricing pipeline — ported from carwash-service's {@code pricing} feature (V1 baseline
 * mirror; the barbershop migration seeds no {@code SERVICE_CHARGE} row, so resolution falls through
 * to zero for that rule family, same as carwash does before a service-charge row is seeded).
 *
 ...
 *
 * <ul>
 *   <li>{@code VAT_BARBERSHOP} — the barbershop-sale tax rate in basis points. Whether this is PPN
...
public class TaxChargeRule extends Auditable {
  ...
  /**
   * Rule key for the barbershop-sale tax (VAT/regional service tax — regime is SME-gated).
   *
   * <p>Named {@code KEY_VAT} (mirroring carwash-service's constant name) — the barbershop regime is
   * more likely PPN (VAT) or a general regional service tax, but that too is SME-gated — hence the
   * neutral name.
   */
  public static final String KEY_VAT = "VAT_BARBERSHOP";
  ...
  /**
   * The rule family key: {@code VAT_BARBERSHOP} or {@code SERVICE_CHARGE}. New rule types may be

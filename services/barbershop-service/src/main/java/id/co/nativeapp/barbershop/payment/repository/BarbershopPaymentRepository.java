package id.co.nativeapp.barbershop.payment.repository;
...
/**
 * Spring Data repository for {@link BarbershopPayment}. Carries no manual {@code WHERE company_id}:
 ...
 */
public interface BarbershopPaymentRepository extends JpaRepository<BarbershopPayment, UUID> {
  ...
  @Query(
      value =
          ...
          SELECT id AS id, ticket_id AS ticket_id, business_id AS business_id,
            ...
            FROM barbershop_payment
           WHERE ticket_id = :ticketId
          """,
      nativeQuery = true)
  Optional<BarbershopPaymentView> findViewByTicketId(@Param("ticketId") UUID ticketId);
}

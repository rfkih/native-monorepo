...
/**
 * Wires the ADR 0049 P4 operator-session signing key for barbershop-service — the OFFLINE VERIFIER
 * side (mirrors employee-service's {@code OperatorTokenSigningConfig}, the MINTER side, and
 * restaurant-service's identical P2 config): binds the shared {@code libs/security} {@link
 * OperatorTokenProperties} (the externalized HMAC key, env/Vault-sourced) — NOT auto-registered by
 * {@code libs/security} itself, so this service opts in explicitly here — and builds the singleton
 * {@link OperatorTokenSigningKey} that {@link OperatorSessionFilterConfig} registers {@code
 * OperatorSessionFilter} with.
 *
 ...
 * (mirrors payment-service's {@code PiiEncryptionConfig} L2 guard, employee-service's identical
 * minting-side guard, and restaurant-service's P2 verifying-side guard). The {@code
 * application.yml} default exists only so the local dev stack and the test suite have a
 * deterministic round-trip key — but it is a well-formed HMAC key sitting in a public repo, and
 * this is the key barbershop-service trusts to attribute a sale's seller (and therefore its
 * commission). A production/UAT boot that forgot to inject {@code NATIVE_OPERATOR_TOKEN_KEY} must
 * fail loudly at startup, never silently verify operator-session tokens against a public key.
 */
@Configuration
    ...
    if (!devProfile && DEV_PLACEHOLDER_KEY.equals(properties.signingKey().strip())) {
      throw new IllegalStateException(
          "native.operator-token.signing-key is still the committed dev placeholder under a"
              ...
              + " against a public key (ADR 0049 P4)");
    }

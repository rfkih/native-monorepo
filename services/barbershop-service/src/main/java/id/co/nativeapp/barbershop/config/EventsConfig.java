...
import id.co.nativeapp.barbershop.entitlement.service.ProjectionEntitlementLoader;
 ...
 *       SaleRecorded}/{@code MetricPublished} row commits atomically with the ticket. Stamped with
 ...
 * </ul>
 *
 * <p>Unlike carwash-service (which still wires a second, legacy {@code wash.service.PostOutboxHook}
 * for its grandfathered {@code POST /washes} path), barbershop-service has no legacy revenue path —
 * the ticket checkout is the ONLY producer — so only the ticket-feature test seam is wired here.
 */
@Configuration
@EnableConfigurationProperties(BarbershopProperties.class)
public class EventsConfig {
  ...
  public EntitlementCache entitlementCache(
      StringRedisTemplate redisTemplate, BarbershopProperties properties) {
   ...
   * id.co.nativeapp.barbershop.ticket.service.TicketWriter TicketWriter} test seam). Declared
   * {@link ConditionalOnMissingBean} so the ticket atomicity test can supply a throwing hook in its
   * own context without a bean-definition clash.
   */
  @Bean

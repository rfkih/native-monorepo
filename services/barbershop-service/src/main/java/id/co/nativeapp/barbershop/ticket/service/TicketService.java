...
/**
 * Orchestrates barbershop ticket quote/checkout/capture/get — the carwash-service {@code
 * TicketService} contract, verbatim (ADR 0024).
 *
 ...
 * the configured {@code barbershop} module before doing any work. A company NOT entitled gets
 * {@link NotEntitledException} → {@code 403}, and NO ticket row / event is produced. {@link
 * #getById} is deliberately UNGATED — a read carries no revenue/side-effect risk (the same
 * asymmetry {@code CatalogService} applies to catalog reads).
 *
 * <p><strong>Checkout idempotency (the carwash {@code TicketService} contract, verbatim).</strong>
 * Not {@code @Transactional} itself: {@link #checkout} runs the create in {@link TicketWriter}'s
 * own {@code REQUIRES_NEW} transaction so a concurrent-collision conflict — {@link
...
public class TicketService {
  ...
  /**
   * Checks out a ticket idempotently — GATED by the barbershop entitlement. A retry — sequential OR
   ...
   *
   * @throws NotEntitledException if the bound company is not entitled to the barbershop module (→
   *     403)
   */
  ...
  /**
   * Captures a PENDING digital tender's payment — GATED by the barbershop entitlement.
   *
  ...
  public TicketResponse capture(UUID ticketId) {
    ...
    try {
      ...
      if (current.payment() != null
          && BarbershopPayment.Status.CAPTURED.name().equals(current.payment().status())) {
        return current;
      }

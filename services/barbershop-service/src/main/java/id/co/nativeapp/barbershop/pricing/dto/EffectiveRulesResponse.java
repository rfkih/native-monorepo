package id.co.nativeapp.barbershop.pricing.dto;

import java.time.LocalDate;

/**
 * {@code GET /api/v1/barbershop/pricing/effective-rules} response — the tax + service-charge rules
 * effective TODAY (UTC), resolved the same way {@link
 * id.co.nativeapp.barbershop.pricing.service.TaxChargeService TaxChargeService} resolves them at
 ...
 * Ported verbatim from carwash-service (ADR 0024).
 *
 ...
 * @param taxBp the {@code VAT_BARBERSHOP} rate in basis points, {@code 0} if unseeded
 * @param taxRuleVersion the resolved {@code VAT_BARBERSHOP} rule's version label, or {@code null}
 * @param taxProvenance the resolved {@code VAT_BARBERSHOP} rule's provenance ({@code

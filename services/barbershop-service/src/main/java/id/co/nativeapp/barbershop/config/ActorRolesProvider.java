 ...
 * every authenticated request (ported verbatim from carwash-service's {@code config} package; see
 * {@code TenantContextHeaderFilter} in the gateway module).
 *
 * <p>This is a thin web-tier adapter that lets {@link
 * id.co.nativeapp.barbershop.outletref.service.OutletAccessGuard} — which is not a controller and
 * has no direct access to {@code HttpServletRequest} — consult the caller's roles without threading
 * them as parameters through the entire call chain or changing the shared {@code TenantContext}
 * record.
 *
 * <p>In the {@code dev} profile the {@link id.co.nativeapp.barbershop.config.DevTenantFilter}
 * trusts the inbound {@code X-Company-Id}/{@code X-Actor} headers (it does not itself read {@code
 ...
 * cashier/attendant with no roles header is denied just like one with an empty role).
 */
@Component

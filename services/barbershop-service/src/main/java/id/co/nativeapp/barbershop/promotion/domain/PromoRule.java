 ...
 * id.co.nativeapp.barbershop.promotion.service.PromotionEngineService PromotionEngineService} resolves
 * at checkout/quote time. Maps to the {@code promo_rule} table (V3__promotions.sql). Ported verbatim
 * from restaurant-service via carwash-service (identical DDL, identical entity shape).
 *
 ...
 * rather than a {@link id.co.nativeapp.money.Money Money}/{@code MoneyEmbeddable} pairing — a
 * percent-type rule legitimately carries no amount and vice versa, so the "amount, always present"
 * invariant a {@code MoneyEmbeddable} implies does not hold here (rule 8 — never a float; still true,
 * just not wrapped in {@code Money}).
 *
...
public class PromoRule extends Auditable {
   ...
   * same idiom as {@code MoneyEmbeddable}), even though — unlike those — this column is nullable.
   */

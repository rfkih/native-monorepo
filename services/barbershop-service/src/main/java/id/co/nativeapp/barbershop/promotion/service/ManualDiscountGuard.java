 ...
 * owner} or {@code manager} role — ported verbatim from restaurant-service via carwash-service (ADR
 * 0026), adapted to barbershop's single checkout write path ({@code TicketWriter.create} — no
 * park/pay-parked/pay-bill flows exist here, ADR 0023 decision 1, preserved by ADR 0024).
 *
 * <p><strong>Empty-roles-pass semantics</strong> (mirrors barbershop's own {@code
 ...
 * real cashier/barber token IS denied. A headerless request means the gateway-less dev recipe or a
 * direct service-layer test, where {@code OutletAccessGuard}'s grandfather clause applies the same
 * trust.
 */
@Component

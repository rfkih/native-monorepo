package id.co.nativeapp.barbershop.entitlement.messaging;
...
/**
 * Loads barbershop-service's CONSUMER copies of the {@code EntitlementGranted} / {@code
 ...
 * Confluent / Schema Registry serde (the same approach finance-service / employee-service /
 * carwash-service use for their consumed events). barbershop-service owns its own consumer view of
 * each contract (full name {@code id.co.nativeapp.events.entitlement.EntitlementGranted} / {@code
 * .EntitlementRevoked}, matching docs/EVENT-CATALOG.md); a contract test asserts these copies stay
...
public final class EntitlementEventSchemas {
  ...
  /** The parsed reader schema for {@code EntitlementGranted} (barbershop's consumer copy). */
  public static Schema grantedSchema() {
    return Holder.GRANTED_SCHEMA;
  }

  /** The parsed reader schema for {@code EntitlementRevoked} (barbershop's consumer copy). */

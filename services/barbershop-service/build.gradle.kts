// barbershop-service — Phase 2 of the POS-parity program (ADR 0024): the 3rd vertical. System of
// record for barbershop operations (service / addon / staff), and the third SaleRecorded producer
// so finance-service consolidates barbershop revenue alongside restaurant + carwash.
//
// A deployable Spring Boot 4 app cloned from the carwash-service SHAPE: REST web, JPA persistence
// over PostgreSQL with a Flyway baseline, auto-applied RLS tenant isolation (rule 5), Auditable
// audit columns, the transactional outbox (rule 3), and lean observability. It inherits the
// platform auto-configs (RLS/auditing/JWT/RFC-7807) from libs/tenant + libs/security — NO copied
// config.
//
// It is BOTH a producer and a consumer:
//   * PRODUCES SaleRecorded (the SAME Avro contract finance already consumes — one shared
//     SaleRecorded.avsc in libs/contracts; business_id = the barbershop outlet) and
//     MetricPublished (service_count at the declared outlet grain, plus the conditional
//     employee-grain sales_amount), both via the transactional outbox in one @Transactional unit
//     with the ticket insert.
//   * CONSUMES EntitlementGranted / EntitlementRevoked into a LOCAL entitlement projection that
//     backs a DB-backed EntitlementLoader for the shared libs/entitlement-check gate (every ticket
//     write is REJECTED with 403 when the company is not entitled to the "barbershop" module), and
//     the EmployeeChanged / AssignmentChanged events into a LOCAL staff read model — all idempotent
...
dependencies {
    ...
    //   money  — the Money value type (integer minor units + ISO-4217 currency); every ticket
    //            amount and the MetricPublished sales_amount value are Money, never a float.
    ...
    // The shared Redis-cached entitled?(company, module) gate. barbershop-service supplies the
    ...
    // Web (/healthz + the /api/v1/barbershop/** POS surface) + JPA persistence.
    ...
    // Bean Validation (Jakarta Validation + Hibernate Validator): @Valid on every POS request body
...
// restaurant-service / carwash-service / entitlement-service / employee-service do it.
tasks.named<Test>("test") {
    systemProperty("spring.profiles.active", "dev")
    // Security review W-4: the gift-card code-derivation HMAC key. MUST be the IDENTICAL value in
    // every other service's test task (loyalty/restaurant/carwash-service) — see

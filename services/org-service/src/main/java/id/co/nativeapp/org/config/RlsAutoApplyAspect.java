...
/**
 * THE auto-applied RLS guarantee (rule 5), copied from the service-template shape.
 *
 ...
 *
 * <p>This is load-bearing for the M1.2 tenant bootstrap: the create-company flow binds the NEW
 * company_id as the session tenant, so this aspect sets {@code app.current_tenant} to that id and
 * the RLS {@code WITH CHECK} passes on the bootstrap insert of the company + first org_unit +
 * outbox row.
 *

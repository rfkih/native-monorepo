...
/**
 * AES-256-GCM authenticated encryption for PII/credential columns (rule 6). One instance holds the
 * 256-bit key sourced from {@link PiiEncryptionProperties}; {@link PiiAttributeConverter} delegates
 * to it so the device-credential password column is ciphertext at rest and decrypts back
 * transparently on read.
 *
 * <p>Mirrors {@code employee-service}'s {@code config.PiiCipher} byte-for-byte — each service holds
 * its own instance keyed by its own {@code native.pii.key} (database-per-service, rule 1; no shared

...
/**
 * Externalized configuration for PII/credential column-level encryption (rule 6 — the
 * device-credential password is column-level encrypted, never logged).
 *
 ...
 *
 * <p>Deliberately its OWN key — distinct from every other service's {@code native.pii.key} (each
 * service holds its own instance, database-per-service, rule 1); org-service does not share
 * employee-service's key or vice versa.
 *

...
/**
 * Read projection over the {@code group_membership} row — only the columns the
 * list-group-members read path needs, never the {@code Auditable} bookkeeping.
 *
 ...
 * its own {@code projection} package — a read model is neither the write-side {@code domain}
 * entity nor a request/response {@code dto}.
 */

 ...
 *       request body (a missing/blank {@code name}, {@code baseCurrency}, {@code defaultLanguage},
 *       or a missing {@code firstBusiness}) → {@code 400} with a machine-readable {@code errors}
 *       list of {@code {field, message}} (one entry per violated constraint), which the React forms
 *       map back to each field;
 *   <li>{@link IllegalArgumentException} — a value the domain rejects, most importantly a bad/blank
 *       ISO-4217 base currency code (the {@code Company} aggregate validates {@code base_currency}
 *       via {@code java.util.Currency.getInstance}, which throws {@code IllegalArgumentException}
 *       for an unknown code) or an unknown {@code OrgUnitType} → {@code 400} with the message as
 *       {@code detail};
 *   <li>{@link TenantAccessDeniedException} — a path {@code companyId} that does not match the
 *       bound tenant scope (a confused-deputy attempt) → {@code 403}; and
...
public class ApiExceptionHandler {
  ...
  /** A domain-rejected value (e.g. an unknown ISO-4217 base currency) → 400. */
  ...
  public ProblemDetail handleTenantAccessDenied(
    ...
    ProblemDetail problem = problem(HttpStatus.FORBIDDEN, "tenant-mismatch", request);

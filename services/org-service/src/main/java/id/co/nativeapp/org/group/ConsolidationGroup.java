 ...
 * the {@link id.co.nativeapp.org.company.Company} base currency uses ("Settings live at creation";
 * CLAUDE.md). The field is mapped {@code updatable = false} (so even a Hibernate flush cannot
 * rewrite the column), there is NO setter that reassigns it, and the only way to set it is the
 * all-args constructor — which validates it is a real ISO-4217 code via {@link

 ...
 * Spring Framework 7 (ADR 0004 / ADR 0008 — the fleet rollout).
 *
 ...
 * 3.0.x line). This test boots the real service and asserts the docs endpoint is genuine OpenAPI
 * JSON, not Base64, AND that springdoc actually scanned the live controllers (the company /
 * org-unit paths are present). So a future Boot/springdoc bump that breaks doc generation fails the
 * build here instead of silently shipping a broken {@code /swagger-ui}.
 *
...
class OpenApiDocsSmokeTest extends PostgresRlsTestBase {
  ...
  void apiDocsIsValidOpenApiJsonNotBase64AndDocumentsTheLiveEndpoints() {
    ...
    // springdoc scanned the live controllers: the company + org-unit endpoints are documented.
    assertThat(body).contains("/api/v1/companies").contains("/api/v1/org-units");
  }
}

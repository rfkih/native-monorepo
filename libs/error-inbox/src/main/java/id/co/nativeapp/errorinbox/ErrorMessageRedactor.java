...
public class ErrorMessageRedactor {
   ...
   * numbers, phone numbers, NPWP (15-digit tax ID), and any other long numeric identifier. Single
   * space / hyphen / dot separators <em>inside</em> the run are tolerated and consumed, so a
   * formatted account ({@code 1234 5678 9012}), phone ({@code 0812-3456-7890}), or dotted NPWP
   * ({@code 12.345.678.9-012.000}) is masked as a whole — not just contiguous digit runs. A "short"
   * number (e.g. an amount {@code 12345}, or an IP {@code 192.168.1.1} = 9 digits) is NOT masked.
   * Erring toward masking is the safe direction for a non-RLS ops table whose sole PII mitigation
   * is redaction.
   */

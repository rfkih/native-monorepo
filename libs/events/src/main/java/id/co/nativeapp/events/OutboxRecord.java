 ...
 *
 * <p>The relay reads these and emits them to Kafka; the {@link OutboxWriter} produces them.
 * {@code payload} is the serialized event body (typically Avro binary from {@link AvroSerde}).
 * {@code headers} is an optional text/JSON blob for transport metadata — it must NEVER carry
 * PII or secrets. {@code companyId} keeps every event tenant-scoped (CLAUDE.md rule 5).
 *

...
/**
 * Avro serialization helper: encodes/decodes a {@link GenericRecord} to/from {@code byte[]}
 * using Avro's compact {@link BinaryEncoder}/{@link BinaryDecoder}, and checks schema
 * backward-compatibility before any schema evolves.
 *
 * <p>The concrete Native event schemas (e.g. {@code SaleRecorded}) arrive in later milestones;
 * this helper works against any {@link Schema}/{@link GenericRecord} so it can be exercised
 * with an inline test schema today and reused unchanged once real schemas exist.
 *
 * <p>CLAUDE.md rule 7 / ARCHITECTURE.md: event schema changes are backward-compatible ONLY.
 * {@link #isBackwardCompatible(Schema, Schema)} encodes that gate — a reader using the new
 * schema must be able to read data written with the old schema.
 */
public final class AvroSerde {
     ...
     *
     * @param record the record to encode; its {@link GenericRecord#getSchema() schema} is the
     *     writer schema
    ...
    /**
     * Deserializes Avro binary using Avro's schema-resolution rules, mapping data written with
     * {@code writerSchema} onto {@code readerSchema}. This is how a consumer reads an older
     * event with its newer (backward-compatible) schema.
     *
     * @param bytes        the encoded data
     * @param writerSchema the schema the data was written with
    ...
    /**
     * Checks whether {@code newSchema} can read data written with {@code oldSchema} — i.e. the
     * change from old to new is backward-compatible.
     *
     * @param oldSchema the previously-registered (writer) schema
     * @param newSchema the proposed (reader) schema
     * @return {@code true} if every record written with {@code oldSchema} is readable under
     *     {@code newSchema}
     */
    ...
    /**
     * Runs Avro's full compatibility analysis (reader = {@code newSchema}, writer =
     * {@code oldSchema}). Returns the raw result so callers can surface the specific
     * incompatibility (e.g. in a contract-test failure message).
     */

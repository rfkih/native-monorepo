...
/**
 * A functional interface that returns the W3C {@code traceparent} string for the CURRENT span,
 * or {@code null} when no span is in scope.
 *
 ...
 * outbox writer can capture a traceparent without a hard runtime dependency on Micrometer —
 * callers that have no {@code Tracer} (tests, DB-only modules) pass a {@code () -> null} lambda.
 *
...
public interface TraceparentSupplier {
  ...
  /**
   * Returns the W3C {@code traceparent} for the active span in the form
   * {@code 00-<32-hex-traceId>-<16-hex-spanId>-<01|00>}, or {@code null} when there is no active
   * span.
   */
  String get();

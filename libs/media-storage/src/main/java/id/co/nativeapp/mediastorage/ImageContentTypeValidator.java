...
/**
 * Verifies an image upload's ACTUAL bytes against its DECLARED {@code Content-Type} by magic-byte
 * signature — the declared header is client-supplied and therefore untrusted: a spoofed header
 * (e.g. an executable renamed {@code photo.jpg}) must not be accepted just because the client
 * claims {@code image/jpeg}.
 *
 * <p>This is the fleet's ONE copy of the check (ADR 0048): it unifies the two functionally
 * identical per-service validators it replaces — employee-service's {@code
 * ReceiptContentTypeValidator} (ADR 0030 §8) and payment-service's {@code
 * QrImageContentTypeValidator} (ADR 0045) — differing only in the exception thrown, now the shared
 * {@link UnsupportedImageTypeException}.
 *
 ...
 * </ul>
 *
 * <p>Stateless and framework-light: no Spring dependency, usable from any layer.
 */
public final class ImageContentTypeValidator {
  ...
  private ImageContentTypeValidator() {}
   ...
   * {@code Content-Type} is guaranteed-canonical. A {@code null} declared header (a multipart part
   * without one — browsers always send it, some API clients don't) is tolerated when the bytes
   * themselves match a whitelisted signature: the magic bytes are the authority, the header only
   * cross-checks it.
   *
   * @return the canonical {@code CONTENT_TYPE_*} constant detected from the bytes
   * @throws UnsupportedImageTypeException if the actual bytes are unrecognised, or disagree with a
   *     non-null declared header (→ 422 at the service edge)
   */
  public static String validate(String declaredContentType, byte[] data) {
    ...
    if (detected == null || (normalizedDeclared != null && !detected.equals(normalizedDeclared))) {
      throw new UnsupportedImageTypeException(declaredContentType, detected);
    }
    return detected;
  }

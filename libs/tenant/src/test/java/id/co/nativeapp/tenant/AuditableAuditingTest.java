 ...
 *
 * <p>Persists a {@link SampleEntity} and asserts that Spring Data JPA auditing populated {@code
 * created_at}/{@code updated_at} and that {@code created_by}/ {@code updated_by} came from the
 * actor.
 *
 * <p>Two complementary checks share this class:
 *
 * <ul>
 *   <li>{@link #populatesActorFromTenantScope()} uses the <em>real</em> {@link TenantContext} scope
 *       (the production path: the actor flows from the bound scoped value through {@link
 *       TenantAuditorAware}); and
 *   <li>{@link #populatesActorFromMockedSecurityContext()} swaps in a <em>mocked</em> {@link
 *       AuditorAware} to prove the wiring reads whatever the security context supplies — the
 *       literal "mocked security context" the build plan calls for.
 * </ul>
 */
@DataJpaTest
...
class AuditableAuditingTest {
  ...
  /**
   * Nested test that replaces the {@link AuditorAware} bean with a Mockito mock — the explicit
   * "mocked security context" from the acceptance criteria — to prove the auditing pipeline writes
   * exactly the principal the security layer reports, independent of {@link TenantContext}.
   */
  @DataJpaTest
  @Import(JpaAuditingConfig.class)
  static class WithMockedSecurityContext {

    @MockitoBean private AuditorAware<String> auditorAware;
